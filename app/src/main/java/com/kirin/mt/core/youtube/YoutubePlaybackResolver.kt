...
class YoutubePlaybackResolver(
   ...
   */
  private fun webPlayerSubtitleTracks(player: JsonObject): List<PlaybackTrack> {
    ...
    return captionTracks.mapNotNull { it as? JsonObject }
      .mapIndexedNotNull { index, track ->
        val baseUrl = track.stringOrNull("baseUrl") ?: return@mapIndexedNotNull null
        val name = track.obj("name")
        val vssId = track.stringOrNull("vssId")
        PlaybackTrack(
          id = index,
          baseUrl = forceWebVttUrl(baseUrl),
          backupUrls = emptyList(),
          bandwidth = 0,
          codecs = "",
          width = 0,
  ...
  private suspend fun buildWebSabrFallback(
    ...
    val subtitleTracks = webPlayerSubtitleTracks(player)
    if (subtitleTracks.isNotEmpty()) {
      Log.i(
        Tag,
