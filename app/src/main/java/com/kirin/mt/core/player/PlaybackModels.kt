...
data class PlaybackInfo(
  ...
  /**
   * YouTube 字幕轨(WebVTT URL,来自 NewPipe info.subtitles)。播放器经 [SubtitleTracks.attach] 以
   * **懒加载**方式并入 MergingMediaSource(P11-120;prepare 期零读取,未选中不发请求)。
   * 非 YouTube/无字幕为空。
   */
...
data class PlaybackTrack(
  ...
  /**
   * P11-120:字幕轨显示名(NewPipe `SubtitlesStream.getDisplayLanguageName()`,如「中文（简体）」)。
   * 仅字幕轨非空;空时 UI 回落 [languageCode]。
   */
  val displayName: String? = null,
  /**
   * P11-120:是否 YouTube 自动生成(asr)字幕。UI 追加「自动生成」后缀;轨选择用 role flags
   * (asr=`ROLE_FLAG_SUPPLEMENTARY` / 人工=`ROLE_FLAG_CAPTION`)区分同语言重轨(如 en 与 en-asr)。
   */
