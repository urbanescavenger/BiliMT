...
internal interface PgcIndexOption {
  val display: String
}

/** 排序方式 */
internal enum class IndexOrder(val id: Int, override val display: String) : PgcIndexOption {
  UpdateTime(0, "更新时间"),
  DanmakuCount(1, "弹幕数量"),
  PlayCount(2, "播放数量"),
  FollowCount(3, "追番人数"),
...
/** 排序方向 */
internal enum class IndexOrderType(val id: Int, override val display: String) : PgcIndexOption {
  Desc(0, "降序"),
  Asc(1, "升序");
}

/** 类型 */
internal enum class SeasonVersion(val id: Int, override val display: String) : PgcIndexOption {
  All(-1, "全部"),
  FeatureFilm(1, "正片"),
  Movies(2, "电影"),
  Other(3, "其他");
  ...
  companion object {
    fun getList(pgcType: PgcType): List<SeasonVersion> = when (pgcType) {
...
/** 配音 */
internal enum class SpokenLanguage(val id: Int, override val display: String) : PgcIndexOption {
  All(-1, "全部"),
  OriginalSoundtrack(1, "原声"),
  ChineseDubbing(2, "中文配音");

  companion object {
    fun getList(pgcType: PgcType): List<SpokenLanguage> = when (pgcType) {
...
/** 地区 */
internal enum class Area(val id: Int, override val display: String) : PgcIndexOption {
  All(-1, "全部"),
  ...
  companion object {
    fun getList(pgcType: PgcType): List<Area> = when (pgcType) {
...
/** 完结状态 */
internal enum class IsFinish(val id: Int, override val display: String) : PgcIndexOption {
  All(-1, "全部"),
  Finished(1, "完结"),
  Serialization(0, "连载");

  companion object {
    fun getList(pgcType: PgcType): List<IsFinish> = when (pgcType) {
...
/** 版权 */
internal enum class Copyright(val id: Int, override val display: String) : PgcIndexOption {
  All(-1, "全部"),
  Exclusive(3, "独家"),
  Other(1, "其他");

  companion object {
    fun getList(pgcType: PgcType): List<Copyright> = when (pgcType) {
...
/** 付费状态 */
internal enum class SeasonStatus(val id: Int, override val display: String) : PgcIndexOption {
  All(-1, "全部"),
  Free(1, "免费"),
  Paid(2, "付费"),
  Prime(4, "大会员");
  ...
  companion object {
    fun getList(pgcType: PgcType): List<SeasonStatus> = when (pgcType) {
...
/** 季度 */
internal enum class SeasonMonth(val id: Int, override val display: String) : PgcIndexOption {
  All(-1, "全部"),
  January(1, "1月"),
  April(4, "4月"),
  July(7, "7月"),
  ...
  companion object {
    fun getList(pgcType: PgcType): List<SeasonMonth> = when (pgcType) {
...
/** 出品方 */
internal enum class Producer(val id: Int, override val display: String) : PgcIndexOption {
  All(-1, "全部"),
  BBC(1, "BBC"),
  NHK(2, "NHK"),
  SKY(3, "SKY"),
  ...
  companion object {
    fun getList(pgcType: PgcType): List<Producer> = when (pgcType) {
...
/** 年份（Anime/GuoChuang） */
internal enum class Year(val str: String, override val display: String) : PgcIndexOption {
  All("-1", "全部"),
  ...
  companion object {
    fun getList(pgcType: PgcType): List<Year> = when (pgcType) {
...
/** 发布时间（Movie/Documentary/Tv） */
internal enum class ReleaseDate(val str: String, override val display: String) : PgcIndexOption {
  All("-1", "全部"),
  ...
  companion object {
    fun getList(pgcType: PgcType): List<ReleaseDate> = when (pgcType) {
...
/** 风格 */
internal enum class Style(val id: Int, override val display: String) : PgcIndexOption {
  All(-1, "全部"),
  Movie(-10, "电影"),
  Original(10010, "原创"),
  Comic(10011, "漫画改"),
  ...
  companion object {
    fun getList(pgcType: PgcType): List<Style> = when (pgcType) {
