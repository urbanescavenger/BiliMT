...
- [v3.1.0-alpha.2](#v310-alpha2)
- [v3.1.0-alpha.1](#v310-alpha1)
- [v3.0.12](#v3012)
- [v3.0.11](#v3011)
- [v3.0.10](#v3010)
- [v3.0.9](#v309)
...
**B站 动态流补上一大块内容 + 播放器两处 ABR 缺口** —— 这一版两条线。①B站 动态页此前**一条图文动态都看不到**,原因不是接口不给,是我们自己在三处丢掉了(取数只请求视频类动态、解析只认视频/直播、模型也只有视频字段);现在图文动态可以正常显示,视频动态卡也照官方版式补齐了。②YouTube 播放器两条 ABR 缺口(连续重载不降档、挂死期间没有评估点)按真机日志修掉。

### 变更

- **B站 图文动态现在能看了**(P11-180 ~ P11-183)。此前动态页一条图文都没有:取数只请求视频类动态(服务端因此根本不返回图文)、解析只认视频/直播两种类型、数据模型也只有视频字段。这一版把三处都补上,并按官方版式做卡片:
  - 图文卡片:作者 + 正文(默认 6 行折叠,可展开)+ 图片九宫格(1 图整宽、2 图并排、3 张以上取前 3 张并在右下角标「+N」)+ 转发/评论/点赞计数;
  - 正文里的**自定义表情(表情包)内联成小图**渲染,不再显示成 `[保佑]` 这类占位文字;
  - 视频动态卡补齐官方版式的三处:UP 的**动态正文**(此前只显示视频标题,动态文案整段丢失)、缩略图左下角的**时长 + 播放 + 弹幕**(时长带底板,播放/弹幕为压在底部渐变上的纯文字)、卡底的**转发/评论/点赞**计数行。
- **UP 主页「粉丝/关注」恒显示 0**(P11-179)。真机日志抓到根因:打开 UP 主页时同一秒会发出三四个 space 接口请求,B站 频控回 `-799 请求过于频繁`;而这一路的"重试"只是换个签名密钥后 **65ms** 就再发一次(等于没有退避),于是资料请求整体失败,页面把统计静默显示成 0(TV 端「粉丝:0 关注:0」、移动端「粉丝 0」)。修:退避策略统一收口并补上 `-799`,且统计接口成功时不再被连坐丢弃。
- **YouTube 连续重载且不降档**(P11-177)。真机日志显示每次升档后,水位急救降档会被一条恒真的带宽判据挡住,只剩超时看门狗把整场重来,于是 60~90 秒重载一轮。修:判据改看「实测吞吐 + 余量」。真机复测:全天看门狗仅触发 1 次,降档路径多次正常开火。
- **YouTube 挂死期间没有任何评估点**(P11-178)。满缓冲排空的那 40 多秒里播放器阻塞在取段调用上,既没有评估点、挂死证据又被之后的成功样本抹平,只能等看门狗重载。修:把静默挂死按轨道记成独立证据喂给 ABR,冷却目标档改记「饿死瞬间正在播的那一档」。

### 已知

- **图文动态的「转发」「专栏」类型暂不渲染**(接口会返回,当前静默丢弃,转发在样本里约占一成);正文里的话题 / @ / 网页链接暂不可点跳转。
- **「点图看大图」与「点卡片进详情页」尚未实现** —— 按官方客户端的点击行为对齐是下一步。
- P11-178(挂死证据进 ABR)与 P11-179(UP 主页统计)待真机复测;P11-177 已真机复测通过;图文动态相关改动已在真机逐条复测(九宫格「+N」角标、表情内联、覆盖行、计数行)。

---

## v3.1.0-alpha.1

**焦点、重载、备份三件事一起收口** —— 这一版的主线是你连报的三个体验问题:从播放列表起播后返回焦点会丢、YouTube 高画质播放中途反复重载、WebDAV 备份要连试很多次才成功。三个问题都拿到了真机日志、定位到具体机制并做了修复;其中焦点那条改到了根上(数据结构层面),不再只是补丁。

### 变更

- **从播放列表起播、返回后不再丢焦点,也不再重新加载列表**(P11-171 / P11-172 / P11-175)。真机日志显示完整链条:起播瞬间详情页整页被卸载 → 返回时列表是空的、必须重新拉取 → 那段时间里既没有可聚焦的行,焦点还会落到左侧头像并**直接把用户送进「我的」页**。本版三层修:①返回时先把焦点落在「返回」按钮上,不再出现整页无焦点;②恢复期间不再让头像抢走焦点(此前只有部分返回路径做了这层保护);③**把列表数据提到播放器之上存活** —— 返回时列表与翻页进度原样还原,不再重新加载,焦点直接回到你起播的那一行。
- **YouTube 播放中途反复重载**(P11-173)。真机日志里两场同签名:播放器在突发测速的鼓励下从 720p 一路爬到 1440p,而链路实际撑不住(单笔请求要 20 秒以上),缓冲耗尽 → 看门狗把整场重来 → 重来后又爬回同一档。本版按三个方向修:①**重载后给「刚上过的那一档」加 90 秒冷却**(此前只有最高档有此保护),不再重载后 40~90 秒内又爬回同一堵墙;②**缓冲见底时把请求上限收紧到 8 秒**(而非 18 秒),让播放器在缓冲耗尽之前先降档自救,而不是等看门狗把整场重载;③**起播档与显示对齐设置里的「起播画质」**(此前网页取流路用「默认画质上限」,默认不限时会把会话绑到 2160p,屏幕显示 2160p 而实际在放 720p)。
- **WebDAV 备份「连不上服务器,连试很多次才成功」**(P11-174 / P11-176)。日志一上线就给出答案:那台服务器空闲后**第一笔请求要 15 秒以上**才回(边缘/源站冷启),而探测只有 8 秒一档 —— 冷启时必败,手动重试撞上「已经热了」的一次才成。本版:①探测改三档 8s → 15s → 25s,一次操作内足以熬过冷启;②**App 启动时后台先替你把连接热好**(仅已配置时),把冷启挪出你按键那一刻 —— 真机实测第一档 3.6 秒即通;③探测层补上逐次日志(此前这一层完全没有日志,失败无迹可查),并把「405 也算可达」纳入判定(部分 WebDAV 服务器拒绝对目录发起 GET)。

### 进展与已知问题

- **待你验证**:播放列表起播 → 返回那条路径,改动后**我这边还没有拿到真机日志**,请复测确认(期望:不再出现重新加载、焦点回到起播那一行;日志应出现 `playlist-detail restore … confirmed=true rows=[N]`)。
- **未修**:焦点落到左侧头像后 autoConfirm 直接打开「我的」页 —— 本版只在「有恢复在身」时抑制(已覆盖所有返回路径);你主动用方向键把焦点移到头像上时,仍会按你的设置自动打开「我的」页。
- **未解**:重载循环里「ABR 被突发测速拉高」这一环仍在(缓冲健康闸按你的决定本轮未做);另外「单笔响应 10~30MB 且客户端整包读取」这条(疑似重载的更深层原因)**只完成了取证方案**,未实施。
- **未证实**:是否存在「一次返回被处理成两级」的迟到返回事件 —— 本轮新增的日志尚未在真机上取到对应样本。
