...
- [v3.0.10](#v3010)
- [v3.0.9](#v309)
- [v3.0.13-alpha.10](#v3013-alpha10)
- [v3.0.13-alpha.9](#v3013-alpha9)
- [v3.0.13-alpha.8](#v3013-alpha8)
- [v3.0.13-alpha.7](#v3013-alpha7)
- [v3.0.13-alpha.6](#v3013-alpha6)
- [v3.0.13-alpha.5](#v3013-alpha5)
...
**上一版(alpha.9)的预取撤销没生效** —— 真机复测发现它挂在了故障时**根本不会被走到**的那条路上。本版把它搬到真正会走到的地方,并补上丢弃流量的取证。

### 变更

- **预取改为「一发现没拿到数据就撤销」(P11-137)**。alpha.9 的撤销挂在"取下一块数据"的入口上,但故障发生时加载器正卡在**重试同一个分段**的循环里,那个入口**一次都不会被调用**(实测 32 秒里只有 1 次),于是整场没打出一条撤销日志、暴风照旧:一次 12 次重试、每次白下约 7.6MB(合计 ≈91MB),缓冲从 29.5 秒被吃空到 5.8 秒,卡顿 2.6 秒。现在撤销挂在**每一笔重试都会经过**的位置:**只要本次响应里没有我们要的分段,立刻撤销预取** —— 下一笔重试就能把分段拿回来,不必干等 30 秒窗口自然过期(那之前每笔都在白下 8MB)。
- **新增诊断:被丢弃的流量取证**。日志会打出 `discarded media: XB of YB (itags=[…])` —— 量化每笔响应里"下载完又整段丢掉"的字节量,用来决定带宽统计口径要不要跟着改。**本版只观测,不改口径。**
- **诊断可读性三项**:播放状态日志补上状态名(`playerState=3(READY)` 而不是只打数字 —— 这正是之前复盘读反、把正常播放当成卡死窗口的原因);两个重试预算的日志分开喊名并带上限(此前都写 "counter reset",分不清清的是哪一个)。
...
- **服务端偶发改推其它清晰度 / 跳过所请求的分段**:这条仍未处理,与预取是两回事(见 docs §34.3 / §34.4)。
...
- **预取在缓冲变薄时立即撤销**(P11-136)。此前的预取窗口只看一次:进入时缓冲够厚(≥20 秒)就开启,之后 30 秒内每个请求都会向服务端点名索要「下一档」的数据,哪怕缓冲已经塌到 0。真机两场实测的后果是:**服务端照办**(每个响应约 **7.87MB 全是下一档的数据**),而当前只认正在播放的格式,于是整段丢弃 —— 更糟的是响应预算被这些数据占满,**当次真正需要的分段挤不进来**,连着 6 次重试、每次再下 8MB,最后服务端会话被重置、重来一遍。两场「连续 32.8 秒 / 25 秒零可用画面」的时间窗与预取窗口**逐帧吻合**。现在进入线仍是 20 秒,但缓冲跌破 **15 秒**就立即撤销,不再与正在播放的清晰度抢响应预算。
...
- **移动端播放器不读「默认播放倍速」**:`MobilePlayerScreen` 把起播倍速写死 1.0x,该设置在移动端一直失效,单独跟进修复。
- **服务端偶发改推其它清晰度 / 跳过所请求的分段**:这仍是上面那类「零可用画面」的诱因之一(本次修的是我们自己的预取抢占,服务端行为本身仍未处理)。
...
- **ABR:一段饥饿只降一档**(P11-135)。真机实测:缓冲漏干复播时水位只剩 **5.8 秒**,而 ABR 在接下来 **5.7 秒的正常播放**里连做了 **4 次降档**(720p→480p→360p→240p→144p)——每次切档要 1.5~3.4 秒拉新档首个数据、期间不放任何画面,于是仅剩的 5.8 秒被三次纯开销切档吃光,**6 秒后再次卡住(3.48 秒)**;落到 144p 后更是 **112 秒**再也升不回去,而带宽、持续带宽、缓冲水位全部远超上一档的门槛(缓冲峰值 34.8s、多次 ≥30s)。原因有二:①水位急救的「仍在下漏」判据每次评估重新判一遍、且**没有降档后宽限**,同一段饥饿被算了四次账;②级联沿途把 720p/480p/360p/240p 逐档各记一笔 180 秒冷却,而升档只允许逐级爬,下一级(240p)被锁住等于**唯一出口被封 180 秒**。现在同一段饥饿只降一档,水位回到阈值以上才允许下一次;真饿仍由带宽估计通道兜底逐级下探,不受影响。真机已验:降一档后即打出「suppressed」不再级联,该次切档耗时 2.855 秒、被水位覆盖,**切换前后均未卡顿**。
...
- **服务端偶发改推其它清晰度 / 跳过所请求的分段,数据被整段丢弃**:实测播放中请求某个视频分段时,服务端改推了**另一个清晰度**的数据(或干脆不给这个分段),而白名单只认已选中的轨道,于是每次完整下载约 8MB 后整段丢弃——**32.8 秒**拿不到可用视频数据(这是上面第一项那次卡顿的起因),另一次同样形态卡了 4.8 秒。本轮改动不会阻止第一次降档(那是正确反应),只让它止步一档;服务端行为本身待单独处理。
- **移动端播放器不读「默认播放倍速」**:`MobilePlayerScreen` 把起播倍速写死 1.0x,该设置在移动端一直失效,单独跟进修复。

## v3.0.13-alpha.7

**设置页新增「YouTube 设置」折叠分组 + 解码器/默认倍速拆成 B站·YouTube 两份**(P11-131)；另含 WEB-SABR 全移动身份、起播锁高、清晰度菜单、升档预加载四项 YouTube 侧改动(P11-127 ~ P11-130)。

### 变更

- **设置页：新增「YouTube 设置」可折叠一级分组**(TV + 移动端同款)。此前 YouTube 项散在两处——TV 端 2 项在「播放设置」、5 项在「YouTube 内容」节；移动端 3 项在「播放设置」、3 项埋在「YouTube SABR 实验」折叠区。现全部收拢为一个分组：默认画质、起步画质、默认倍速、解码器、频道管理、内容地区、Piped 实例、启用 Piped 后端、播放优先级。TV 端默认**展开**(折叠若组头焦点异常会让整组不可达)，移动端沿用既有惯例默认**折叠**。
- **解码器、默认播放倍速各拆成两份独立值**。此前 B站 与 YouTube 共用一个值，改一边影响另一边。现 YouTube 有自己的一份，**首次改动前沿用旧的共享值**——升级后不碰设置的话观感与之前完全一致，在 YouTube 那两行动过一次之后才各自独立。
- **清晰度菜单只列分辨率**(P11-129)：SABR 路径按 height 去重(同一分辨率下的多个 codec / 帧率变体合并成一项)，标签只留 `1080p` 这样的分辨率；同一分辨率用哪条轨由「YouTube 解码器」设置决定。
- **起播档锁高搬进选轨内部**(P11-128)：消除「松 cap 必重建」——此前调整画质上限会重建样本队列并整丢约 1.16s。
- **升档预加载**(P11-130)：在当前请求里带上「下一档候选」，请服务端把它的 init 与段一起推回来缓存，目标是切档那一刻直接命中缓存、不再现拉。
- **WEB-SABR 转全移动自洽身份**(P11-127)：采集页与 `/player` 统一走移动 UA(Android WebView 改不了 Client Hints，伪装桌面必然自相矛盾)，会话恒用移动 clientInfo，token 换移动 minter，并重新启用 status=2 的同步 token 刷新；harvest 优先取带真 token 的 SABR POST，冷启桩(10B)只作兜底。

### 已知问题

- **升档预加载(P11-130)真机取证显示未生效**：候选档只写进了请求的 `preferredVideoFormatIds` 第二位，服务端每个响应只推主格式、不推候选档(日志里候选 itag 只出现在 prefetch 那三行本身，零 FORMAT_INIT / 零 MEDIA_HEADER)。升档仍是冷启动现拉，实测 720→1080 约 2.9s、1080→1440 约 6.6s、1440→2160 约 14.3s，与改动前的构建同档。该场播放因缓冲够深没出现可见卡顿，但机制本身未生效，继续跟进。
- **移动端播放器不读「默认播放倍速」**：`MobilePlayerScreen` 把起播倍速写死 1.0x，从未消费该设置——移动端这一项一直是失效的。本次只保证新拆的两份设置口径一致(都失效)，单独跟进修复。
- **解码器「自动」的偏好顺序在两个平台不一致**：B站 侧为 AV1 > H.265 > H.264(界面描述即此)，YouTube 侧实际为 H.264 > VP9 > AV1 > H.265。新版为 YouTube 单列了描述文案，B站 那句未动。

## v3.0.13-alpha.6

**TV 播放器 UI 全面对齐官方客户端 + 三源底栏统一**(P11-124)

### 变更
- **B站 播放器版式重排**(对齐官方客户端):①**顶部三行**——标题(28sp、最多 2 行)→ 元信息单行(`UP主 · 播放量 · 弹幕数 · 点赞 · 收藏 · 投币 · 发布于 <时间>`,以 ` · ` 串联、值为 0 的项整项跳过、超长直接裁掉)→ **动作行**(点赞 / 收藏 / 投币 / 稍后再看 / 评论;图标 + 文字、**不带计数**——计数在上一行);三连由此**从底栏搬走**。②**底栏重排**——最左是倍速文案 `1.0x`,其后一排**裸图标**(UP主页 / 画面旋转 / 刷新 / 字幕 / 弹幕开关 / 播放序列 / 播放列表 / 相关视频 / 画质 / 设置),按内容宽紧挨、整行靠左;右下角的「在看 / 弹幕 / 画质」状态文案整块去掉。③**焦点扩成三级**:控制行 ⟷ 进度条 ⟷ 顶部动作行(↑ 逐级向上、↓ 逐级回),任何时刻只有一处高亮。
- **底栏新增三个入口**:「**刷新**」(重新解析并重载当前视频,**保持当前播放点**)、「**画面旋转**」(按一次顺时针 +90°:0→90→180→270,旋转后画面**铺满整屏**)、「**播放序列**」(列表播放 / 单视频循环两态,图标 `repeat` / `repeat_one` 按状态切换)。
  - 旋转踩过一个硬坑:**Android 的 SurfaceView 不支持旋转变换**(只吃尺寸/位置变化,180° 同样无效)——这是「按下没反应」的根因。修法:旋转态改由 **TextureView** 承载(`PlayerView` 的 `surface_type` 只能从 XML 传,故新增 `res/layout/view_player_texture.xml`),并改由**视图自己的 `View.rotation`** 驱动(Compose 的 `graphicsLayer` 在这条链上根本落不下去);0° 时仍回默认 SurfaceView,不牺牲默认播放性能。
  - 单视频循环用 `REPEAT_MODE_ONE`,并在 prepare 完成后**重设一次**(刷新 / 切清晰度 / 切音轨都会重建会话,不重设会丢态);循环时播完链路(下一 P / 自动推荐 / 播完退出)被显式挡掉。
- **画质显示改成文字 `HD`、字幕改成 CC 图标**(未选字幕轨时图标压暗;真实档位与当前字幕轨改由无障碍描述承载);**暂停图标从屏幕正中挪到右下角**。
- **YouTube 底栏对齐同一套**(11 项:`1.0x | UP主页 | 画面旋转 | 刷新 | 字幕 | 播放序列 | 播放列表 | 相关视频 | 评论 | HD | 设置`;YouTube 无弹幕故没有弹幕开关,评论留在底栏);**影视库(TVBox)/ IPTV / 红果 统一为 8 项**(`1.0x | 画面旋转 | 刷新 | 播放序列 | 播放列表 | 相关视频 | HD | 设置`——去掉 UP主页与字幕:这三个源没有 UP 信息、多数视频也无字幕轨;字幕仍可从设置面板进)。三份项集**显式维护**,不再受枚举声明顺序影响(顺带修掉「倍速文案排到设置之后」的顺序错位)。
- **清理**:删掉旧底栏那套渲染(`PlayerBottomOverlay` / 玻璃方块按钮 / 状态文案 / `TvProgressBar` 等,净约 -480 行,均全仓确认 0 引用);「在看」在线人数的**空转轮询**(状态 / job / 定时器 / 接口调用)整条清除——它已无消费者,数据源保留在仓库未接回;直播人气是另一条路,未动。

### 真机判据
- B站:顶部三行齐全(标题 2 行 / 元信息 / 动作行),底栏 11 项且 `1.0x` 在最左、右下无状态文案;↑ 从控制行走到进度条再到动作行,动作行内 ←→ 移动、OK 触发(点赞直投 / 投币弹确认 / 收藏夹 / 稍后再看 / 评论),↓ 逐级退回。
- 旋转:按一下顺时针 90°,再按 180 / 270 / 0;旋转后铺满整屏(只看得到源中间约 1/3 横条,这是「铺满」的必然裁切);旋转态下弹幕 / 控制层 / seek 预览**不跟转**。
- 播放序列:切到单视频循环后播完应循环,不跳下一 P、不自动推荐、不自动退出。
- YouTube 与影视库 / IPTV / 红果:底栏项集与顺序如上,旋转 / 刷新 / 播放序列在它们上面同样可用;焦点为两级(无顶部动作行)。
- 「在看」在线人数在点播播放器任何源都不再出现。

## v3.0.13-alpha.5

**移动端「会员」角标对齐 TV + 自定义播放列表补角标**(P11-121)

### 变更
- **新增共用组件 `SourceBadge`**:BiliPink 粉底圆角胶囊 + 白粗体,对应 TV `VideoCard` 的 `VideoBadge`。此前 TV 与移动端的源角标各写各的、无共用组件,移动端三处是各自内联的裸 `Text`(位置与样式都不一致)。
- **封面卡片角标改右上**(`MobileVideoCard` 紧凑卡 + 动态卡):原先贴**左上且是裸白字、无底色**,压在亮色封面上看不清;现贴右上、粉底胶囊。首页/推荐/搜索/动态/收藏/历史/直播/稍后再看/离线/相关视频等所有走 `MobileVideoCard` 的出口一并生效。
- **在线播放列表详情行**:角标左上 → **右上**;「已看完」右上 → **左下**(已看完时不画进度条,左下本就空着,右下仍留给时长)。原先「右上撞车所以退到左上」的约束就此解掉。
- **自定义播放列表详情行补角标**:这是用户报的「播放列表里不显示会员角标」——该行封面 `Box` 里**从来没有写过 badge 渲染**(只有进度条 + 已看完)。现补上右上角标;**编辑模式自动隐藏**(右上让位给既有的「✕ 移除」按钮)。
- **播放器「播放列表后续」行补角标**:YouTube 播放器「简介」Tab 里这段单列缩略图行原先是裸 `AsyncImage`,整块无任何角标;从自定义播放列表起播时它显示的正是列表内容,一并补上。
- **直播角标 `LiveBadge` 按用户选择保持左上**(TV 的直播角标走 `VideoBadge` 在右上,此地两端仍不一致,如需统一可后续再改)。
- 真机判据:会员专属视频在卡片/在线播放列表/自定义播放列表上角标均应在**右上**且为**粉底胶囊**;自定义播放列表进编辑模式角标消失、退出恢复;已看完的列表行角标在**左下**,不与右下时长重叠。
...
- **同批并入的可播性修复**(本 tag 提交范围内):
  ...
  - **会员专属视频不再过滤**(P11-100):展示进 feed 并打「会员」角标;播放列表详情页视频行补源角标渲染(TV 右上/移动端左上;移动端已于 alpha.5 统一改为右上粉底胶囊,P11-121)。
