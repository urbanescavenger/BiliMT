...
- [v3.0.10](#v3010)
- [v3.0.9](#v309)
- [v3.0.13-alpha.25](#v3013-alpha25)
- [v3.0.13-alpha.24](#v3013-alpha24)
- [v3.0.13-alpha.23](#v3013-alpha23)
- [v3.0.13-alpha.22](#v3013-alpha22)
- [v3.0.13-alpha.21](#v3013-alpha21)
- [v3.0.13-alpha.20](#v3013-alpha20)
- [v3.0.13-alpha.19](#v3013-alpha19)
- [v3.0.13-alpha.18](#v3013-alpha18)
- [v3.0.13-alpha.17](#v3013-alpha17)
- [v3.0.13-alpha.16](#v3013-alpha16)
- [v3.0.13-alpha.15](#v3013-alpha15)
- [v3.0.13-alpha.14](#v3013-alpha14)
- [v3.0.13-alpha.13](#v3013-alpha13)
- [v3.0.13-alpha.12](#v3013-alpha12)
- [v3.0.13-alpha.11](#v3013-alpha11)
...
**稳定版:YouTube 播放链路大修 —— 「自己搭的会话」打通(新播与续播都走通),并连根修掉四类「画质掉下去就回不来」;TV 播放器 UI 全面对齐官方客户端、三源底栏统一;移动端会员角标补齐。** 本版整合 v3.0.13-alpha.1~25 全部改动。

### 变更

**YouTube 播放**

- **「自己搭的会话」真正打通**(alpha.3, 11~17, 23~24,P11-118/139/140/156/158~160,`docs/youtube-web-sabr.md` §5.9~§5.11):YouTube 的播放令牌此前一直用我们自己铸的那一枚,服务端始终当成"临时占位"处理,十几秒后就拒绝并要求换令牌。逐层取证(身份、材料、令牌形态、超时)之后定位到**令牌的来源不对** —— 改成「会话仍是我们自己的(所以服务端供给**我们要的**清晰度),只把播放令牌换成页面上自己铸的那一枚」,并补上两个"放弃得太早"的判据与一处形同虚设的超时上限。真机实测:**连续播放 13 分 24 秒、服务端校验 108 次全部通过、281 个分段、零卡顿零重载**,画质正常升到 1080p;**「继续观看」的续播**同样走通。中途走过一段「借用浏览器材料」的弯路,实测它只提供浏览器当时选中的那两档(我们要的档拿不到),该路线已整段停用。
- **SABR 续播「满缓冲黑屏死锁」根治**(alpha.1,P11-96,`docs/youtube-sabr-abr-upshift-notes.md` §31/§32):续播点落在分段中间时读取被永久锁死 —— 缓冲显示已满、解码器却不喂帧、永远到不了 READY。按官方实现补全协议后,死锁位 5.5 秒起播。
- **画质「掉下去就回不来」的四类原因**(alpha.20~22, 25,P11-151/152/153/164/167/168):①**降档冷却一刀切 180 秒且不分原因**(拖动进度、切换来源造成的降档也被当成"这个清晰度扛不住")→ 按原因定时长,供给不足 180→90 秒、拖动 / 手选只锁 20 秒,缓冲恢复即提前解除;②**「只在服务端给过的档位里选」这条约束被静默套用到所有会话** ⇒ 不去请求 1080p 就永远拿不到 1080p,**死锁在 720p**(手机端「停在 144p 不回升」同源)→ 该约束改为只对采集会话生效;③**「缓冲满就试探升档」跳到了远超实测容量的档**(自报 18.6 Mbps、实测容量约 9 Mbps),切过去喂不动 ⇒ **画面定住、声音靠缓冲继续 33 秒** → 试探升档不得超实测容量的 1.5 倍,且 1080p 及以下的单请求等待上限从 40 秒收到 18 秒;④**开关字幕 / 暂停 / 满缓冲的排空段被误判成"带宽不足"** → 分别修成「选档重建时继承当前档」「长时间需求性空闲不计入带宽估计」「带宽撑得住当前档就不开水位急救这一枪」。
- **音轨可切 + 字幕覆盖三条取流路**(alpha.4,P11-119/120):过去点选音轨毫无变化(主链的签名里根本没有这个参数),并且恒播英语配音轨;字幕整块下线后改为**懒加载**挂载 —— 每条字幕轨在 prepare 期零读取、失败即弃,字幕的任何网络问题都够不到主源。
- **播放稳定性三项**(alpha.8,P11-133/134/135):水位急救降档改为「一次冻结只降一档」;移动端补回 stall 看门狗与 SABR 请求的整调用超时;YouTube 解码器拆成自己的值域(含 VP9),梯子锚点跟随。
- **修「无效流量」**(alpha.9/10,P11-136/137):升档预取会占用服务端响应的预算,把正在播放的那一档的数据挤掉(实测白下 8MB 又整段丢弃、连续 25~33 秒拿不到画面)→ 预取在缓冲变薄时立即撤销。
- **一次约 99 秒的静默卡顿**(alpha.25,P11-162)、**慢启动不再白等 18 秒**(alpha.25,P11-166):逐行核对了 FreeTube 的实现补上三处缺失的超时(其中最长的一处是自己代发请求时没设连接 / 读取超时);「多久没有新字节」的阈值从 18 秒收到 8 秒 —— 实测这种停顿**重试只要 4 秒就能成功**。

**界面与交互**

- **TV 播放器 UI 全面对齐官方客户端 + 三源底栏统一**(alpha.6,P11-124):B站 版式重排为顶部三行(标题 / 元信息 / 动作行),三连从底栏搬到标题下方;新增**刷新**、**画面旋转**(顺 90→180→270,旋转态铺满整屏)、**播放序列**(列表播放 / 单视频循环)三个入口;画质显示改 `HD`、字幕改 `CC` 图标;YouTube 底栏对齐同一套,影视库 / IPTV / 红果 统一为 8 项。
- **移动端会员角标补齐**(alpha.5,P11-121 + alpha.25,P11-169):新增共用组件 `SourceBadge`(粉底胶囊),卡片角标从左上裸白字改为右上胶囊(首页 / 推荐 / 搜索 / 动态 / 收藏 / 历史 / 直播等所有出口一并生效);在线播放列表行的「已看完」让位到左下;自定义播放列表行与播放器「播放列表后续」行补上此前**完全没有**的角标;**频道页与 UP 空间投稿列表**的视频行也补上(此前从来没画过)。
- **TV 设置页遥控器丢焦点**(alpha.18,P11-148):在设置里上下选择时焦点框消失、方向键全部失效,列表深处的 WebDAV「备份」按不到、弹窗里也选不中任何项。根因是滚完只等一帧就抢焦点,而离屏的行根本没组合 → 改为「滚到目标进入布局后按帧重试」,预算用尽则就近把焦点交给可见行,绝不留空焦点树。
- **YouTube 设置独立分组 + 解码器 / 默认倍速拆成两份**(alpha.7/18,P11-127~131):解码器与默认倍速此前 B站 / YouTube 共用一份,改一边会互相影响;现在各自独立,并新增「YouTube 设置」专区(全移动身份、起播锁高、清晰度菜单、升档预加载);随后按真机反馈把折叠组改成**二级面板**(与首页分区、日志、关于同一套机制)。
- **频道页 / 播放列表返回丢焦 + 网格 D-pad 导航**(alpha.2,P11-98/98b/98c):返回时按记录位置恢复焦点(滚动定位 + 等待布局 + 重试);网格顶行 ↑ / 首列 ← 的边界按键不再逃逸到侧栏头像。

### 进展与已知问题

- **起播仍偏慢**:走「借页面令牌」这条路时要先在一个隐藏浏览器里打开 watch 页(约 17.7 秒),整段起播实测约 51 秒。上一版承诺的**并行化已实测回退**(3 场里只有 1 场成功,并出现协程作用域被提前取消的报错),起播提速改由「静默超时 8 秒」拿回一部分;这仍是下一轮的主要目标。
- **部分清晰度的声明码率是假的**(vp9 系回落成 VBR 峰值:144p 报 4.7 Mbps、720p60 报 18.6、2160p 报 71.6,真值约 0.1 / 2~3 / 25)。本版已止血(升档基准按实测容量夹住)并留下诊断日志,**根因(在抓取层按「内容长度 ÷ 时长」补算真实平均码率)尚未处理**。
- **1440p / 2160p**:默认这条路的清晰度天花板实测为 **1080p**(服务端该场只提供 6 个高度);更高档位的路径状态与上一版结论一致,不在本版改动范围内。
- **弱网下「带宽闸」的另一侧尚未取到证据**:目前的真机回归只覆盖「带宽充足时不该降档」;真实带宽不足时是否照旧降档,需要在低带宽场次复测。
- **本版包含两条并行会话的改动**:TV 设置页焦点修复(alpha.18)与「YouTube 设置」二级面板。
...
**画质不再乱掉** —— 上一版把续播打通之后,反馈集中在另一类症状上:画质会莫名其妙掉到最低档,而且**掉下去就回不来**。这一版连着修了三个不同的原因,并且都拿到了真机回归;另外移动端频道页补上了此前缺失的会员角标。

### 变更

- **开字幕会掉到 144p**(P11-164)。开关字幕会让播放器重新选一次档,而重选时的「初始档」取的是列表最后一个 —— 也就是**最低档**。现在记住当前正在播的档,重选时先回到它。这个问题不只字幕能触发:任何一次选档重建都会踩到,字幕只是最常见的那个。
- **暂停一会儿之后砸到 144p**(P11-167)。暂停时不再拉流,这段空白被带宽估计当成了「网络一个字节都收不到」,于是判定带宽归零、直落最低档。现在把这类长时间的「需求性空闲」(暂停、切后台)与真正的「供给不足」区分开:前者不计入带宽估计。
- **带宽明明够,画质却钉在低档**(P11-168)。这条最隐蔽:播放器升到高画质的那一刻缓冲水位偏低(满缓冲主动停拉的正常排空段),被「水位急救降档」误当成告急,于是**降回去、并把高档禁用 90 秒**;此后每一轮升档都撞同一堵墙。现在给这个急救降档加了一道带宽闸 —— **当前带宽连这一档的码率都撑得住时,不开这一枪**。真饿(带宽确实低于当前档码率)时照旧降档。
- **移动端频道页、UP 空间投稿列表补会员角标**(P11-169)。会员专属视频的角标数据一直是好的,只是这两处的视频行**从来没画过角标**(同一页的 Shorts/直播 tab 和 TV 端都有)。现在复用首页卡那枚粉底胶囊,贴在封面右上角,与全 App 其余位置样式一致。
- **修一次约 99 秒的静默卡顿**(P11-162)。逐行核对了 FreeTube 的实现,补上三处缺失的超时(其中最长的一处是自己代发请求时没设连接/读取超时,阻塞读把线程占满,连兜底都没法触发)。
- **慢启动时不再白等 18 秒**(P11-166)。「首包偶发十几秒一个字节都不发」时,原来会等到 18 秒上限才切断重试;实测这种停顿**重试只要 4 秒就能成功**,所以把「多久没有新字节」的阈值收到 8 秒 —— 停顿 8 秒就立刻重发。
- **内部**:采集到的「冷启占位令牌」不再被兜底路径放行进入会话(P11-165)。

### 进展与已知问题

- **真机回归(带宽闸)**:连续一场约 3 分钟,视频轨**只升降一次**(720p → 1080p)并稳定在 1080p;全程**零降档、零门禁、零卡顿、零重载、零报错**;那唯一一次水位急救降档被带宽闸挡下 —— 正是它没被降档,才没有触发 90 秒禁用,升档随后顺利完成。上一版遗留的「上不去」到此消失。
- **仍未验证的一侧**:目前的证据只覆盖「该挡时挡得住」。**弱网下「真饿时该挡不挡」还没有取到证据**,需要在低带宽场次复测。
- **上一版承诺的「起播并行化」已回退**(P11-161)。实测 3 场里只有 1 场成功、并出现协程作用域被提前取消的报错,整笔回退;起播提速改由上面那条「静默超时 8 秒」拿回一部分。
- **起播仍偏慢**(承接上一版):借令牌环节约 17.7 秒,是下一轮的主要目标。
- 本次回归场的清晰度天花板是 **1080p**(服务端该场只提供 6 个高度);1440p/2160p 的路径状态与上一版结论一致,不在本版改动范围内。

## v3.0.13-alpha.24

**「继续观看」也能用自己搭的会话播了** —— 接着上一版那条线收尾。上一版让 YouTube 改走我们自己搭的会话(实测连续播 **13 分 24 秒**、1080p);这一版补上**续播**,并修掉两个「放弃得太早」的判据和一个「上限形同虚设」的配置问题。

### 变更

- **续播失败之一:把好页当坏页**(P11-158)。借令牌前要先在一个隐藏浏览器里打开 watch 页。实测那次页面**其实已经完整加载**(82 万字节),但「8 秒内没收到页面完成信号」就被判成空白页丢弃;而同机成功的那一次,页面完成信号是 **8.1 秒**才来的 —— **差 0.1 秒**。现在改成**看证据**:文档不是空的就继续等,不再只凭「时间到没到」判死。
- **续播失败之二:只肯等 6 秒**(P11-159)。页面放**前贴片广告**时只发「占位令牌」,真令牌要等内容起播才铸出来。原实现「拿到占位令牌后再等 6 秒」就放弃,而实测真令牌是在 **8.5 秒**后出现的 —— **差 2.5 秒就漏掉**。现在改为**等满本轮采集窗口**(窗口本身有预算上限,不会无限等)。
- **修「上限形同虚设」**(P11-160)。播放请求走的是通用网络配置(15 秒超时,那是给小接口定的),导致我们给播放设的「按清晰度 18 秒 / 40 秒」上限**根本轮不到生效**:服务端偶发慢启动时(正常首包 3.5~7.2 秒,慢的时候 16.5 秒一个字节都没发),请求会在 15 秒被切断、整个会话重建。现在把播放请求的超时与那个上限对齐。

### 进展与已知问题

- **实测结果**:连续续播 3 场,**2 场直接成功、1 场偶发失败后自动重试成功**;服务端校验 `status=1` ×32、被拒 **0** 次,末笔画质升到 **1080p / 1440p**。新播那边(上一版)是连播 13 分 24 秒。
- **起播仍偏慢**:续播实测约 **51 秒**才出画面(其中借令牌 17.7 秒,前面还有约 22 秒的准备)。下一版做并行化。
- **仍未根治**:vp9 系清晰度的「真实平均码率」需要在抓取层补算(取内容长度÷时长)。
- 降档/升档偶有摆动(上一版处理过的「画面定住」那条线的邻居问题),与本版改动无关。
- **本版同时包含**并行会话的设置页改动。

## v3.0.13-alpha.23

**「自己搭一条 YouTube 会话」这条路真正打通了**。真机实测:连续播放 **13 分 24 秒**、服务端校验 **108 次全部通过**、281 个分段、**零卡顿零重载**,画质正常升到 **1080p**。此前同一套链路每个视频只能撑十几秒就被服务端要求换令牌。

### 变更

- **YouTube 的播放令牌改成「借页面自己铸的那一枚」**(P11-156)。根因是**令牌的来源不对**:我们一直用**自己铸造**的令牌,而服务端始终把它当"临时占位"处理(实测第一笔请求就被降级),于是十几秒后彻底拒绝;而**页面自己铸造**的那一枚(真机实测 88 字节)一直被接受。历史数据也一致 —— 能长时间播放的那几次,用的都是页面铸的令牌。
  现在:**会话本身仍是我们自己的**(所以服务端会供给**我们要的清晰度**,不会像"整份借用浏览器那次会话"那样只给少数几档),**只把播放令牌换成页面铸的那一枚**。
- **借不到就退回原路,失败仍立刻让位**(P11-156)。采集没拿到真令牌时,回落成自己铸的令牌(与改动前行为一致);任何一次失败都立刻切到 NewPipe 主链 —— 不会出现"连试多条路线、期间一直不能播"的情况。

### 进展与已知问题

- **起播多花约 20 秒**。借令牌前要先在一个隐藏浏览器里打开 watch 页、等播放器把令牌铸出来;首次尝试实测会撞上"空白页"而被判超时,重建后重试才成功(两次页面就绪实测都在 **~8 秒**,而放弃阈值也正好是 **8 秒**,卡在边界上)。下一轮专门处理。
- **仍未根治**:vp9 系清晰度的「真实平均码率」仍需在抓取层补算(取内容长度÷时长),属单独一轮。
- 降档/升档偶有摆动(alpha.22 处理过的「画面定住」那条线的邻居问题),与本版改动无关。
- **本版同时包含**并行会话的设置页改动。

## v3.0.13-alpha.22

**修「看着看着画面定住、声音还在」**。真机实测:画面定住 **33 秒**直到手动退出,而声音一直在放。三条原因叠在一起,这一版全部处理。

### 变更

- **清晰度升档不再跳到「喂不动」的档**(P11-153)。「缓冲满就试探升档」这条机制**凭缓冲满就放行**,这次跳到了一个**远超实测容量**的档(它自报 18.6 Mbps,而实测容量只有 ~9 Mbps)——切过去之后数据喂不动,而旧档已缓冲的视频又被丢弃 ⇒ 画面定格、声音靠自己的缓冲继续。现在**试探升档不得超实测容量的 1.5 倍**。
- **挂死时的等待时间缩短**(P11-153)。单个请求原本有 **40 秒**上限(那是给 4K 大段留的余量),于是低清晰度请求也要白等 40 秒才切走 —— 你就是在这个窗口里退出的。现在 **1080p 及以下收紧到 18 秒**,1440p/4K 仍保留 40 秒(大段本来就可能传很久)。
- **修「有些清晰度的声明码率是假的」**(P11-153)。部分清晰度(vp9 系)拿不到真实的平均码率,于是**回落成了 VBR 峰值**——实测:144p 报 4.7 Mbps、720p60 报 **18.6**、2160p 报 **71.6**(真值约 0.1 / 2~3 / 25)。选档与升档全部建立在这份假数据上,这正是上面那条跳档的土壤。本版:①**升档后的带宽基准改为按实测容量夹住**,不再锚到失真的声明值;②给这类回落**加了一行诊断日志**,以便把范围查清后从根上补算。

### 进展与已知问题

- **仍未根治**:vp9 系清晰度的「真实平均码率」需要在抓取层补算(取内容长度÷时长),属于单独一轮;本版先止血并留下证据(日志里会明确列出哪些清晰度回落成了峰值)。
- **仍未解决**:WEB-SABR 仍不能作为日常主路(能真播约 17 秒后被服务端要求新令牌);默认已是「单次尝试 + 失败即让位主链」,不影响观看。
- **本版同时包含**并行会话的设置页改动。

## v3.0.13-alpha.21

**修「画面清楚、播放稳定,但清晰度就是上不去」**。真机实测:视频本身有 **1080p / 1440p / 2160p** 可选,带宽也足够(实测 3.8~5.3 Mbps,而 1080p 只需 3.0),缓冲甚至堆到 **43~49 秒** —— 画面却**全程停在 720p**,怎么都不升。

### 变更

- **「只在服务端给过的档位里选」这条约束改为只对采集会话生效**(P11-152)。这条约束原本是为「用采集材料建的会话」设计的 —— 那种会话服务端**只提供浏览器当时选中的档位**,所以要收窄候选。但它此前被套用到**所有**会话,而普通会话服务端点播是**要什么给什么**:于是候选集合永远只剩「已经请求过的那一档」——
  **我们不去请求 1080p ⇒ 服务端也就不推它 ⇒ 集合永远不变 ⇒ 死锁在 720p**(手机端此前「停在 144p 不回升」同源)。现在普通会话恢复自由升档,采集会话仍按老规矩收窄。
- 附带一次编译修复(把「是否采集会话」改由工厂闭包传入 —— 该字段在选择器内部不可见)。

### 进展与已知问题

- **背景**:这条约束是本轮为「采集会话」加的(见 alpha.19 之前那条线),属于**为 A 场景造的约束被静默套到 B 场景**的典型 —— 它的死法看起来不像 bug(播放稳定、只是永远低档),所以拖到现在才被你看出来。
- **仍未解决**:WEB-SABR 仍不能作为日常主路(能真播约 17 秒后被服务端要求新令牌);默认已是「单次尝试 + 失败即让位主链」,不影响观看。

## v3.0.13-alpha.20

**修「清晰度被钉在最低档、怎么都不回升」**。实测现象很明确:缓冲一直有 **25~28 秒**(网络完全够),画面却在约 2 分钟里一直停在 **144p**。

### 变更

- **降档后的「冷却」不再一刀切 180 秒,而是按原因定时长**(P11-151)。原来只要发生一次降档,就把刚离开的那个清晰度**锁 180 秒**,而且**不区分原因** —— 拖动进度、切换来源、会话重建这些操作造成的降档,也被当成「这个清晰度扛不住」的证据。于是掉下去之后就再也升不回来。现在:**只有真正的供给不足**(带宽确实不够)才记长冷却(**180 秒 → 90 秒**);**拖动/手动选档**引起的降档只锁 **20 秒**。
- **缓冲恢复后提前解除冷却**(P11-151):缓冲回到 **20 秒以上**、且实测带宽超过该清晰度的声明码率(×1.1)时,**立即解除**该档冷却(不再死等),日志会打 `cooldown cleared early: …`。
- **每次降档多打一行原因**(P11-151):清晰度从哪到哪、当时的带宽/持续带宽/缓冲、是否被锁、是否刚拖动过 —— 便于继续定位剩余的档位问题。

### 进展与已知问题

- **背景**:这三处属于「降档护栏」的既有机制(快小样本过滤、滑行豁免、降档冷却)之一 —— 前两道实测工作正常(42 秒的空窗里豁免了 33 秒),**问题出在第三道的触发条件过宽、惩罚期过长**;本版只改这一道,不动带宽统计口径(那条经多轮调校,待新日志证据再动)。
- **仍未解决**:WEB-SABR 仍不能作为日常主路(能真播约 17 秒后被服务端要求新令牌);默认已是「单次尝试 + 失败即让位主链」,不影响观看。

## v3.0.13-alpha.19

**把「为取证而改的行为」全部收回开关里**:上一版为了让四条候选路线做对照,把轮换装在了默认路径上 —— 实测发现**每条会话都能建起来、但约 20 秒后会被服务端判死,而且不让位给能播的主链**,导致整场看不到画面。本版默认恢复「单次尝试、失败立刻让位」,**几秒内就能看到画面**;需要对照取证时,开发构建里把开关打开即可。

### 变更

- **对照实验收进默认关闭的开关**(P11-150)。默认:WEB-SABR 只试一次(我们自己的会话 + 自铸令牌,约 2.6 秒、不做采集),失败**立刻**让位给能播的主链;开发构建把开关打开才会依次轮换四条路线。
- **不再回落到已淘汰的令牌来源**(P11-149)。移动端令牌铸造器还没产出时,此前会退回使用桌面挑战链的令牌 —— 实测那个组合会让播放器判定「不可播放」。现在最多等 6 秒,等不到就直接跳过这条路线。
- **采集只拿到「占位令牌」时不再白跑第二次采集**(P11-149):实测白烧 36 秒,而结果一样(仍然只有占位令牌)。
- **四条路线失败时打一行汇总**(P11-149),便于从日志判读。

### 进展与已知问题

- **WEB-SABR 现状(诚实交代)**:它**能真播** —— 实测约 **17 秒**、每笔响应 **100% 可用**、视频与音频两条轨都送到了解码器;但服务端在约 **29 秒**处就会要求一枚它认可的令牌,而我们手上没有(自己铸的被拒;采集腿当天两次尝试都没采到真令牌)。**所以 WEB-SABR 仍不能作为日常播放主路** —— 失败后会立刻换到能播的主链(主链实测 `status=1`、零错误、媒体段 100% 可用)。
- **仍待解决**:如何取得一枚服务端认可的令牌(社区结论指向「铸造时的网页上下文」)。按计划这条线再给 ≤2 个构建,**拿不到「零 `status=3` 且连续播放 >60 秒」就降级为可选档**,精力转回主链与画质/起播。
- **本版同时包含**并行的 TV 改动:YouTube 设置折叠组改成二级面板 + 焦点修复。

## v3.0.13-alpha.18

**这一版修 TV 端设置页的遥控器焦点问题**:在设置里上下选择时会突然「丢焦点」—— 焦点框消失、方向键全部没反应,列表深处的 WebDAV「备份」行因此按不到、弹窗里也选不中任何项。

### 变更

- **修掉「上下一跳就丢焦点」**(P11-148)。设置列表滚动到目标行后,只在下一帧就去抢焦点,而 LazyColumn 里**滚出屏幕的行根本没有被创建**,那一刻抢焦点必然落空;同时原来持焦点的行又被这次滚动回收掉了,于是整个界面**没有任何控件持有焦点**,方向键自然全部失灵。现在改成:先确认目标行真的画出来,再抢焦点,并**按帧重试**;万一仍然抢不到,就把焦点交给旁边一个**确实可见**的行 —— 不再留「无人持有焦点」的状态。
- **同一个毛病在 5 个设置弹窗里一并修掉**(P11-148):WebDAV 配置 / IPTV 源 / TVBox 源 / Piped / 备份还原选择。弹窗刚打开时若第一次抢焦点落空,整个弹窗的方向键就全哑(表现为「无法在 TV 上勾选要备份的项目」),现在会重试到拿到焦点为止。
- **日志补全**:设置页从此有明确的焦点落点轨迹(`BiliMT:Settings` 选行日志,外加焦点诊断里新增的 `[settings]` 区域标签),下次真机若仍丢焦点,日志能直接分出「焦点彻底丢光」还是「焦点跑到别处」。

### 进展与已知问题

- **本版结论由代码判读得出**(本机取不到 TV 日志,近期日志全是移动端),**待真机验证**:一路按 ↓ 应能到 WebDAV「备份」并在弹窗内勾选 + 确认。若仍丢焦点,请把 `BiliMT:Settings` 那几行(尤其 `focus retry exhausted` / `focus fallback`)发回来。
- 「YouTube 设置」折叠分组(P11-131)本身没有错:它只是把散落各处的 YouTube 项并成一个 10 行大块,让原本偶发的跨屏跳转变成日常操作,从而把这个潜伏已久的缺陷放大成必现;本版修的是那个缺陷本身。

## v3.0.13-alpha.17

**这一版是 WEB-SABR 的「对照取证版」**:把四条可能的路线放进同一个包里轮换,让一次真机测试就能分出哪条能通;顺带修掉一个**从没生效过**的选档约束和一个**会挡住后三条路线**的闸门。

### 变更

- **起播选档改为服从「服务端实际会提供的档位」**(P11-146)。此前有一条「只在服务端真会推的档位里选」的约束,但它只在中途生效,**起播那一刻会被「起播锁」夹回一个服务端根本不提供的档位** —— 于是第一笔请求就要一个永远不会来的档,首帧永远等不到,锁也永远不松(结构性死锁)。现在起播锁也服从那份档位表。
- **「选档推动」加了核验**(P11-146):给采集页推清晰度这件事此前注入得太早(那一刻播放器还没起播,日志里读出来是 `unknown`),现在会在 4 秒后**再读一次**,能真正判出「推上去了没有」。
- **修掉一条会挡住对照实验的闸门**(P11-147):WEB-SABR 一旦运行时报错就会给该视频打上「不再重试」的标记,导致对照实验的后三条路线**一次都轮不到**。现在改成:四条路线各给一次机会,都试过之后才恢复「失败即永久让位」。

### 进展与已知问题

- **本版是取证版,行为上刻意多试**:同一个视频反复重试会依次走四条路线(其中两条需要先采集,各等 9~40 秒)。**日常观看不受影响**:任一路线失败都会自动换到能播的路线(alpha.11 起)。
- **实测确认(本版相关)**:「我们自铸的令牌」已被服务端当占位处理,四次复现无例外;而**采集页铸的那枚令牌**在当天早先几轮里拿到过正常状态并真正播放过(两场分别 35 / 115 个媒体块)。本版把「采集页令牌 + 我们自己的会话」也列为一条路线。
- **仍未通**:WEB-SABR 尚未打通;当前能播靠的是 NewPipe 主链(实测 `status=1`、零错误、媒体段 100% 可用)。
- **历史结论修正**:文档里「令牌与播放无关」这条旧结论已标注推翻 —— 它是在「刷新令牌时编码有 bug」的时期得出的,今天的数据与它相反。

## v3.0.13-alpha.16

**这一版是「取证版」**:把两条可疑路线放进同一个构建里对照跑,并加厚日志,好让一次真机测试就能读出结论。

### 变更

- **同一视频内自动对照两条会话路线**(P11-144):同一个视频第一次播放走「采集材料建会话」,被判定失败重试时改走「我们自己的会话」,如此轮换 —— 于是**同一视频、同一网络、相隔数十秒**就能拿到两条路线的同条件对比,不必再拿不同场次的日志拼结论。
- **服务端提示「需要更新播放令牌」时不再换令牌**(P11-144)。此前一收到该提示就把令牌换成新铸的一枚;实测换成的那枚**紧接着就被服务端判为无效**,整个会话随之报废。现在改为**保留当前令牌继续用**,并在日志里记下「刻意不刷新」与令牌年龄 —— 这样下一笔请求的结果就能把「换令牌才是祸首」与「提示本身等于终止」两种情况分开。
- **日志加厚**:每笔响应新增一行摘要(我们请求的档位 / 服务端真正给的档位 / 可用字节),请求日志新增令牌年龄;这两样把「格式对不上」与「令牌不被接受」两类问题在日志里一眼分开。

### 进展与已知问题

- **上一版结论保持**:用自己的会话播能拿到我们的档位与真实媒体段(两条轨都送到解码器),而用采集材料建的会话两个媒体块都拿不到。
- **代价(刻意)**:对照轮换意味着每个视频第一次播放会先走采集路线(要吃 9~40 秒采集,且大概率失败),重试后才走自己的会话。
- **仍未修**:程序自己铸的令牌呈「每次变长」的异常形态(实测 +65 字节/次,涨到 1300+ 字节),且铸造时缺少网页原生上下文 —— 这两条已有明确修法方向(重建铸造环境 + 补上下文),下一版处理。

## v3.0.13-alpha.15

**「用自己的会话播」这条路第一次真正跑通** —— 播放质量与起播速度同时改善。本版把此前充当播放来源的「采集回来的会话材料」整段停用:实测它只提供浏览器当时自己选的那两档(我们的档从头到尾拿不到),而**我们自己的会话提供的就是我们要的档**。

### 变更

- **不再采集、不再用采集材料建会话,播放一律走我们自己的会话**(P11-143)。同一场实测对比:用采集材料建的会话**一个可用媒体块都拿不到**(服务端重复回同一条数据 6 次 / 12 次,全部被丢弃),而自己的会话拿到 **14 个媒体块**、播放器进入 **READY** 并正常出画(720p),音频也正常(4 个音频块)。
- **起播明显变快**:自己的会话从开始到就绪实测 **约 2.6 秒**;采集流程每场要 **9.5 秒以上**(有时 30 秒+)。采集停用后这段时间直接省掉。

### 进展与已知问题

- **上一版(令牌解码)在真机确认生效**:刷新日志回到正确的解码比例(如 `88B → 154B`),整场「令牌无效」判决 **0 次**;本版跑通的自造会话全程 `status=3` **0 次**、`Playback error` **0 次**、无重载。
- **这条弯路的原因已定案**:「用自己的会话播」此前被判死的现象,根源是我们自己刷新令牌时的编码 bug(alpha.13 修掉)—— 于是才绕去用采集材料;现在前提消失,采集停用。
- **仍未动**:① 应用启动时仍会预热采集用的 WebView(实测 3~15 秒),采集停用后它已是纯开销,下轮摘除;② 程序铸出的令牌仍呈「占位形态」(字节数每次 +65 增长、首字节固定),不是签名令牌,单独跟进。
- **WEB-SABR 仍作为可选档**;失败时会自动换到能播的路线(alpha.11 起),日常观看不受影响。

## v3.0.13-alpha.14

**上一版修掉的「令牌被判无效」已确认生效**;这一版处理它暴露出来的下一层问题 —— 采集回来的会话材料**只提供浏览器当时自己选的那两档**,而我们固定要另外两档,两边永远对不上,会话每次都在 20 秒内被判废。本版让「只采到占位令牌」的采集结果**直接作废**,把播放交回**我们自己的会话**(它提供的才是我们要的档位)。

### 变更

- **采集结果只拿到「占位令牌」时不再使用**(P11-142)。网页在选定档位之前会先发一个**占位令牌**(实测 10 字节);等不到真令牌(实测 88 字节)时,此前会把这份占位结果当会话材料用掉 —— 后果有两个:① 服务端全程回「等待验证」(实测一场 114 次),永远进不了正常状态;② 更关键的是它**顶掉了我们自己那条路**(用材料建的会话只提供浏览器那一场绑定的档位)。现在这种结果直接丢弃,播放改用**我们自己的会话**。

### 进展与已知问题

- **上一版修复已确认生效**:服务端的「令牌无效」判决从 22 次降到 **0 次**,令牌刷新日志也回到正确的编码比例(如 `468B → 349B`,恰好 3/4 = 已解码)。
- **用材料建的会话仍有一个未解问题**:即便拿到**真令牌**的材料,它提供的档位仍是浏览器那一场绑定的 —— 实测一场里视频数据正常到达(1.7MB + 3.8MB)、服务端状态也正常,但**我们的音频档从头到尾没有被提供过**,最终仍被判废。本版先绕开它(改用我们自己的会话),材料会话的档位对齐留到下一轮。
- **代价**:丢弃占位材料后,程序会**再试一次采集**,然后才回落到自己的会话 —— 起播大约多 6 秒。
- **采集到的令牌本身还有一个待查问题**:程序自己铸出来的令牌呈「占位形态」(刷新日志里字节数单调增长 `154 → … → 1324`,每次 +65,首字节恒为同一值并带自增计数),不是签名过的真令牌。这条单独跟进。
- **WEB-SABR 仍不作为日常播放主路**;会话被判死时会自动换到能播的路线(alpha.11 起)。

## v3.0.13-alpha.13

**找到「能播,但每隔一阵子就重载一次」的真凶**:服务端要求更新播放令牌时,我们**把令牌的文本形式当成了令牌本身**发出去 —— 服务端必然判它无效,于是整场会话被处决。本版修掉这一处,并把它收敛成唯一入口,防止再分叉。

### 变更

- **令牌更新改为「先解码、再发送」**(P11-141)。服务端要求更新令牌时(会话内状态 `status=2`),我们换上去的那枚令牌此前是**原样的文本**(实测日志里「208 字节」正好等于「208 个字符」—— 完全没有解码),而正确形态是解码后的**字节**。会话**创建**时早就是这么做的,**只有刷新这条路径漏了**:于是服务端每要求刷新一次,我们就把一枚本来能用的令牌换成必被拒的字节,紧接着的每个请求都被判「令牌无效」,会话一路走到死(重载)。
- **刷新日志带上更新前后的字节数**(形如 `120B → 88B`),「有没有解码」从此在日志里一眼可判 —— 这正是本轮判读的依据,也留给下一轮当验收判据。

### 进展与已知问题

- **本轮排除掉一个此前的主要怀疑**:刷新用的令牌铸造器与首次铸造是**同一个实例、没有重建**,访客标识也相同 ⇒ 不是「用另一套身份铸令牌」,问题纯粹出在编码。
- **同一份日志留下三项未修的问题,下轮处理**:①采集材料所建的会话只提供「浏览器当时自己选的那两档」格式(实测两分钟内 275 次被丢弃的格式全是它们),而我们固定要另外两档 ⇒ 两边永远对不上;②「跟着服务端实际提供的档位选档」这一策略会被**起播期的锁档**夹回固定档位,等于从未生效;③采集材料所建的会话在**音频**侧也无法满足我们(材料只供一种音频格式)。反面证据同时拿到:用**我们自己的**会话地址 + 采集来的令牌,格式是正常供的 ⇒ 问题在会话地址(服务端按采集那一场绑定格式供流),不在令牌。
- **WEB-SABR 仍不作为日常播放主路**;会话被判死时会自动换到能播的路线(alpha.11 起),日常观看不受影响。

## v3.0.13-alpha.12

**把 WEB-SABR 会话的「身份与形状」整体对齐到采集回来的浏览器材料**(P11-140)。这一版不改变外观,但它把 WEB-SABR 从「服务端直接判我们不合法」推进到「只剩一个具体缺口」,并首次拿到了**决定性证据**说明后续该往哪走。

### 变更

- **WEB-SABR 会话的身份与形状改为「以采集材料为准」**。此前我们用自己的身份字段去发这个会话,而服务端认定它不合法(每个请求都回「令牌无效」)。现在把采集到的浏览器原始请求里的**身份信息**与**能力/偏好字段**原样带过去 —— 于是**静态部分与浏览器逐字段一致**,而我们没建模的那十来个字段也不必猜(原样透传)。
- **进度与计时仍由我们给当前值**。材料是「采集那一刻」的快照,照抄等于向服务端谎报播放进度;真实客户端也会推进这些值。所以:身份/能力/偏好以材料为准,播放进度/计时以当前值为准,而**播放器实际要播的清晰度**仍由我们决定(照抄材料的选定档会引发重载,已有实测)。
- **「选了哪些轨」的两个字段(轨道类型位、音轨 id)归属修正**:它们属于「请求语义」而不是「身份/偏好」,本版拿回由我们自己填 —— 上一版把它们一并交给材料是分类错了。

### 进展与已知问题

- **服务端判决已升级(关键进展)**:同一视频、同一形态,从 `InvalidPoToken`(身份被判无效、会话直接死)变成 `sabr.no_audio_selected`(一个明确的语义错误)。也就是说**身份与形状这一层已经通过**,剩下的是一个具体缺口。本版已针对该缺口修正了字段归属,待真机验证。
- **判别实验首次拿到结论**:把「浏览器亲手产生、服务端已回 200」的那份材料经我们的通道原样重放,得到 `status=1` + 完整媒体段(755KB)。⇒ **材料是好的,差异在我们自己的会话构造** —— 这直接说明「继续逐字段对齐」是正确方向,不必去动会话轮换或身份这条更重的杠杆。
- **WEB-SABR 目前仍不能直接播**;但会话被判死时会自动换到能播的路线(alpha.11 起),实测同一场靠兜底正常播放 20 段,所以日常观看不受影响。
- **取证工具修了两处**(仅影响日志):我们自己的请求体日志此前被系统单行长度上限截断(丢掉的正好是**身份段**),采集回来的浏览器请求体内容此前**根本没进日志**(只有长度)—— 两处都改为分段输出,「我们的请求 vs 浏览器的请求」逐字段对比才第一次成立。**这份对比正是本轮定位的依据。**
- 历史遗留不变:升档预取本身仍未生效(其副作用已在 alpha.9 止住);移动端播放器不读「默认播放倍速」。
...
- **WEB 会话被服务端拒时,自动换到能播的路线**(P11-138)。此前「WEB-SABR 优先」档下若服务端把会话判死(每个请求都回 `status=3` 令牌无效),程序会**反复重建同一种注定失败的会话**,直到试满重试次数 —— 实测一次 5 个会话、0 个可用分段,表现就是「完全播不了」。现在这种「建起来了但每笔请求都被拒」的失败会被记住,下次直接跳过 WEB-SABR 走 pot-less 主路(实测可播),**不必再手动去设置里切**。
- **取证工具补齐三处**(P11-139):①我们自己的请求体日志此前**被系统单行长度上限截断**(丢掉的正好是请求体最后那段**身份信息**),改为分段输出;②**采集回来的浏览器请求体内容此前根本没进日志**(只有长度),补上对称的分段输出 —— 有了这两样,"我们的请求 vs 浏览器的请求"逐字段对比才第一次成立;③那条「把浏览器材料原样重放、看服务端是否接受」的判别实验此前只在特定失败分支里跑,现在挂到了必然会经过的位置。

### 已知问题

- **WEB-SABR 仍然播不了,但原因首次被定位到字节层**:对比同一份日志里的双方请求体,我们与浏览器在**客户端身份**上不一致 —— 浏览器(采集来源)是 **MWEB**(`clientName=2`、带 `deviceMake/deviceModel`、URL 里 `c=MWEB`),我们的会话是 **WEB**(`clientName=1`、4 字段 clientInfo、URL 里 `c=WEB`)。也就是说,**浏览器那枚令牌是在 MWEB 身份下铸的,我们却以 WEB 身份使用它**,服务端判为无效。这正是文档里早先记下的残留(当时那轮还能通过,所以被判为「不是判据」);现在有字节证据,它升为首要嫌疑。对齐方案已定,但按既有警告**必须连同请求体形状一起改**(身份与形状是耦合的),单开一轮验证。
- **升档预取本身仍未生效**(历史遗留):它要回来的数据仍会被丢弃,「切档命中缓存」未达成;alpha.9 起已止住它抢占响应的副作用。
- **移动端播放器不读「默认播放倍速」**:`MobilePlayerScreen` 把起播倍速写死 1.0x,单独跟进修复。
