...
- YouTube 设置（可折叠一级分组，默认展开）：YouTube 默认画质、起步画质、默认倍速、解码器、频道管理、内容地区、Piped 实例、启用 Piped 后端、播放优先级。其中**默认倍速与解码器**与「播放设置」里的同名项**相互独立**，首次改动前沿用旧的共享值；**解码器是 YouTube 自己的值域**（自动 / VP9 / AV1 / H.264 / H.265，含 B站 侧没有的 VP9），选中哪一族整场就粘哪一族，升档降档不跨族换解码器。
