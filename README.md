...
| tag | 内容 |
| --- | --- |
| v3.0.7-alpha.17 | 声明码率口径修正:`bitrate` 是 VBR 峰值(虚高~60-75%),换 `averageBitrate`(真平均)优先全链路生效(WEB 原生字段/NewPipe 自算 clen÷dur);calib 采样折算整体取消(required=裸声明=实需);顶档 4K sustained 门槛 0.6→×1.1(真平均口径 VBR 尖峰余量);升档重锚裸锚 |
