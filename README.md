...
稳定版：YouTube 播放链路大修（SABR 升档判据改持续带宽根治 pacing 反复横跳、Auto 满缓冲不升 1440 修复、gap 时钟 bug）+ Crashlytics 日志远程回收完整链路（无需 GMS，手动分享恒即时、启动检查送行、注入降噪 56KB 字节预算）+ 双端频道页/排序/搜索结果对齐 B站/YouTube 官方 UI + TV 修退桌面直播后台出声与重开新实例 + 订阅流提速 + 普通卡片真实观看进度条。
