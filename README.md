...
| tag | 内容 |
| --- | --- |
| v3.1.0-alpha.2 | **B站 图文动态 + 播放器两处 ABR 缺口**:①**动态页图文现在能看** —— 此前一条都没有(取数只请求视频类动态、解析只认视频/直播、模型只有视频字段);现按官方版式补:图文卡(作者 + 正文折叠/展开 + 九宫格 1图整宽/2图并排/≥3 取前 3 张 +「+N」角标 + 转发/评论/点赞)、**正文自定义表情内联成小图**(不再是 `[保佑]` 占位文字)、视频动态卡补齐官方三处(UP 动态正文 / 缩略图左下「时长 + 播放 + 弹幕」/ 卡底互动计数行);②**UP 主页「粉丝/关注」恒 0** —— 真因是 space 接口频控 `-799` 而重试只隔 65ms 等于没退避,资料请求整体失败被静默显示成 0,修退避策略收口 + 统计不再被连坐丢弃;③**YouTube 连续重载且不降档**(P11-177,真机复测通过:看门狗全天仅 1 次)与**挂死期间无评估点**(P11-178)按真机日志修。已知:转发/专栏动态暂不渲染、话题/@/网页链接不可点、「点图看大图」「点卡片进详情页」未实现;P11-178/P11-179 待真机复测 |
| v3.1.0-alpha.1 | **焦点/重载/备份三件收口**(P11-171~176):①**从播放列表起播返回不再丢焦点、也不再重新加载列表** —— 详情页数据 hoist 到播放器之上存活(返回即还原列表与翻页进度),配合「返回先落返回键」「恢复期间抑制头像 autoConfirm 把用户送进我的页」「让位判据改看合法落点(修假回调)」「精确恢复失败兜底」共五处;②**YouTube 中途反复重载**三修 —— 重载后给「刚上过的那一档」90 秒冷却(此前仅最高档有)、缓冲见底时请求上限收紧到 8s 让 ABR 先降档自救(起播期除外,避免砍首包)、起播档与显示按「起播画质」对齐(不再屏显 2160p 实播 720p);③**WebDAV 备份「连试很多次才成功」** —— 真因是服务器边缘冷启(空闲后首笔 >15s),探测改三档 8/15/25s + 启动后台预热主机(真机第一档 3.6s 即通)+ 补逐次日志 + 405 视为可达。已知:播放列表返回路径待真机复测;「ABR 被突发拉高」与「单响应 10~30MB 整包读取」未做 |
...
稳定版：TVBox 影视库第三搜索源全系列（设置页填配置 URL 聚合采集站、跨站多线路直链播放、线路=站点选集=线路内分集、配置化删除内嵌白名单、m3u8 死链存活预检自动顺延下一线路、官方播放页源剔除、WebDAV 备份/还原纳入 TVBox 配置）+ B站搜索新增番剧分类（media_bangumi 结果进 PGC 季详情）+ 移动端搜索对齐官方（综合/番剧/UP主三 tab + 排序/时间/时长筛选面板）+ 番剧详情/选集面板对齐官方（两列选集卡+话数分组快捷跳+数据行/角标）+ 移动播放器横屏底栏对齐 B 站官方（⏸⏭/弹幕胶囊/选集/倍速/画质文字）+ 横屏弹幕胶囊打字回显修复 + 更新下载断流修复（callTimeout 按包大小兜底+Range 断点续传+指数退避重试）。
...
稳定版：YouTube SABR Auto 升档体系收尾（满缓冲试探升档根治「播 X 档只能测到 X 档量级容量」的 pacing 测量死锁 + 试探准入/失败冷却/试探熔断三修 + 失败冷却一致化降档即记冷却 + 缓冲读数塌方守门防假 buffer-critical 误降）+ 跨 codec 换解码器防护（VP9 粘性梯子全程单 codec 零重建 + 视频冻结 12s 看门狗）+ 全档零帧渲染黑屏画质熔断 + 4K 死亡行军三层根治 + 移动端竖滑手势亮度/音量 + TV 两次返回彻底退出杀进程 + 动态长按弹窗下线改直进 UP 主页（稍后再看/评论挪进 TV 播放器控制栏）。
