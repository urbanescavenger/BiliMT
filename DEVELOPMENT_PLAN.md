...
| P11-101 | WEB-DASH 兜底:Step 0 撤 HLS 兜底(media3 createFallbackOptions FATAL+段 403)+ Phase 1 判别探针 probeWebDashChain(WEB /player 带 token → dashManifestUrl 参数 → n/s decipher verdict → +pot GET 200);Phase 2 待真机 go/no-go 后接 buildWebDashFromPlayer | Step 0+Phase 1(代码完成,待云编译+真机探针) |
