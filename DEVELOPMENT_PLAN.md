...
当前移动端能力：首页分区网格、动态关注 feed 网格、卡片式设置、短信登录（WebView 托管 B站 登录页，抓 cookie 存 session，点一下即完成）、触屏播放器（播放/暂停/seek/弹幕开关/返回，复用引擎进度上报）。底栏 PGC/Search 仍为"开发中"占位。

### 后续路线图（按优先级）

**P0 —— 补齐四个主 tab**

1. **PGC（影视）tab**：复用 `VideoRepository.getPgcFeed/getPgcSeasonInfo/getPgcIndex` + `PgcType`/`PgcIndexFilters`；类型横滚 tab → 季网格 → 季详情选集 → `toPlaybackRequest(epId=, seasonId=, subType=)` 开触屏播放器（PGC 分支 `MergingMediaSource` 已支持）。
2. **搜索 tab**：复用 `searchVideos`/`getSearchSuggestions` + `SearchHistoryStore`；输入框 + 历史/联想 + 结果网格。

**P1 —— 播放器 v2 + UP 空间**

3. **播放器增强**：画质/倍速/弹幕设置弹窗（复用 `DanmakuSettingsStore` 离散选项）、选集侧栏（`PlaybackVideoMetadata.pages`）、自动连播下一集（`PlayerCompletionPlanner`）、空降助手/在线人数（可选）。
4. **UP 空间**：复用 `getSpaceVideos`/`getSpaceUserProfile`/`checkFollowStatus`/`setFollowStatus`；头像 + 关注 + 视频网格。

**P2 —— 个人内容 + 评论**

5. 历史/收藏/追番（`getHistoryPage`/`getFavoriteFolderVideos`/`getFollowingSeasons`）。
6. 评论（`getComments`）。
7. 动态点赞/稍后再看（`likeDynamic`/`addToView`）。

**P3 —— 打磨与发布**

8. 主题（深色/跟随系统、首页 `HomeThemes` 适配手机）、失败重试、横屏/平板、启动闪屏、应用图标。
9. 2.0.0 稳定后按"大版本才合 main"规则合 `mobile` → `main`（届时打 `v2.0.0` 稳定 tag，二次确认后推）。
