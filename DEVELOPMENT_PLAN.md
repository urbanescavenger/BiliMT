...
| P11-121 | 移动端「会员」角标对齐 TV:新增共用 `SourceBadge`(BiliPink 粉底胶囊)替换三处内联 Text,位置左上→右上(卡片/在线播放列表行/自定义播放列表行);在线播放列表行「已看完」让位到左下;自定义播放列表行补上原先完全缺失的角标(编辑模式隐藏);播放器「播放列表后续」行补角标 | 实施中(代码完成待云编译+真机) |
