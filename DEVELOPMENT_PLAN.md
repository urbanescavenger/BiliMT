...
- **YouTube SABR「挂死期间无评估点」(P11-178 的遗留项)**：ABR 只在 `getNextChunk` 被咨询
  (`DefaultSabrChunkSource.kt:336`)，挂死阻塞期与满缓冲排空期**零评估** —— 真机
  `logs_live_20260923_233549` 里 1440p 从满缓冲漏到 0 的 42s 内一次评估都没有，8s 看门狗整场重载（35s
  黑屏，而成功重试只差 4s）。已用「挂死证据跨评估周期存活」绕开（P11-178①），**未做**的是给看门狗对
  「在途且有字节进展的 fetch」加宽限 —— 需产品权衡（放宽会削弱 P11-167 那条「慢滴挂死」兜底）。
...
| P11-70 | ABR 降档滞回余量 ×0.85(双阈值死区):当前档降档判据放宽到 required×0.85,升档/候选档保持全额——修「1440p↔1080p 临界来回切」(00:01-04 真机:est 15.8M vs 门槛 16.76M 差 6% 即降,3 分钟三轮;declared=真平均后 est 巡航天然骑在门槛 ±10%)。真饿(est<×0.85)照降,水位急救兜底 | 实施中(代码完成待云编译,详见 `docs/youtube-sabr-abr-upshift-notes.md` §18) |
| P11-72 | YouTube 字幕 URL 预检后入列:修「视频转圈加载不出/官方可播」(00:25-26 真机:timedtext 字幕 URL 响应头 81s 不回,MergingMediaSource 等全部 child prepare 拖死主源,SABR fetch 零发送)——NewPipe 字幕逐条 3s callTimeout 探测,不通的丢弃(宁缺毋拖死);SabrAwareDataSource 加 youtube/googlevideo 路由诊断日志 | 实施中(代码完成待云编译) |
| P11-73 | TV YouTube 字幕下线(用户决策:字幕不重要,核心是音视频):PlayerScreen 字幕 MergingMediaSource 整块移除(timedtext 被掐时不再有 child 挂死主源的可能),字幕轨数据仍在 PlaybackInfo 传递留待未来「预检+不阻塞」方案;P11-72 的 resolver 预检撤销(不再消费);SabrAwareDataSource 路由诊断日志保留 | 实施中(代码完成待云编译) |
| P11-71 | TV 播放列表详情页「进入无焦点」修复:初焦单发改验证+重试 —— 单发 `withFrameNanos`+`requestFocus` 在覆盖层环境会撞「FocusRequester is not initialized」(真机 logs_live 23:58:32/00:04:42 同款异常实锤),被 `runCatching` 吞掉后整页永远无焦点且焦点留在被盖住的频道页卡片后面;改为拉焦点后等一帧确认 `screenHasFocus`(根节点 onFocusChanged),未确认继续拉,至多 30 帧,焦点已在屏上立即停;空列表/加载失败初焦落返回 chip。另加 `focusDiag("playlist-detail")` 根标签与频道页播放列表网格 `focusDiag("channel-playlists")`,下次真机日志可直接定位焦点落点 | 实施中(代码完成待云编译+真机) |
| P11-72 | TV 播放列表详情页聚焦高亮改硬渲染:P11-52 玻璃高亮真机不可见(焦点已实锤进屏 GAINED [playlist-detail]、OK 可播,用户仍报整页无高亮;唯一变色的是 P11-69 正在播放标记,不跟焦点走)。列表行+播放全部按钮弃用 playerFocusedLiquidGlassSurface 玻璃链路,改纯粉半透明(35%)直画 background + 3dp 粉边框(不依赖玻璃管线/性能策略,用户选定样例 A);行 onFocusChanged 补 glass=/backdrop= 诊断日志,一次真机日志即可分辨「焦点回调没触发」vs「玻璃没画」 | 实施中(代码完成待云编译+真机) |
| P11-72b | TV 播放列表详情页焦点链补全:r1757(alpha.4)真机日志实锤——初焦重试生效(GAINED [playlist-detail] 于数据到达后 ~100ms),但整个 3.3s 会话**零次 playlist-row 聚焦回调**=焦点全程停在「播放全部」按钮,用户按 ↓ 焦点没到列表行(覆盖层默认焦点搜索丢失,P11-50/51 同款),用户视角=「列表没有变色的是焦点」。改动:①「播放全部」↓ 显式 requestFocus 首行(不交给默认搜索);②首行 ↑ 显式回「播放全部」(成对防丢逃);③节点级诊断补全:playall/back chip onFocusChanged 打日志,初焦重试循环结束打 attempts/confirmed/target 结论行——下轮日志可完整还原焦点落点 | 实施中(代码完成待云编译+真机) |
| P11-72c | TV 播放列表详情页「焦点被简介 clickable 吃掉」修复:r1761 真机日志铁证——initial done 循环自称成功(attempts=1 confirmed=true),但 playall/row/back 三个节点级聚焦回调**全零**,子树 hasFocus=true 的唯一无日志可聚焦节点=简介 Text 的 clickable(TV 参与焦点且视觉零高亮)。改动:①简介 Text focusProperties canFocus=false(简介展开非焦点目标,防它吃初焦与搜索焦点)+诊断 tripwire;②初焦重试成功判据从根 hasFocus 改为「合法落点(播放全部/行/返回 chip)真拿到焦点」(anyRowFocused/backFocused 上报到 screen 层),焦点卡简介时 requestFocus 强行抢回即自愈;③结论日志补 playAll=/row=/back= 三态 | 实施中(代码完成待云编译+真机) |
| P11-72d | TV 播放列表详情页焦点谜团取证+根级自愈:r1762 真机日志——initial done attempts=30 confirmed=false(30 次 requestFocus 全部静默失败),playAll/row/back 全 false,desc 日志未出现,但 screenHasFocus=true=焦点确实在子树里且不在四个已监控节点上。子树可聚焦节点全集已穷举(返回chip/播放全部/视频行),矛盾指向更底层。改动:①requestFocus 失败原因落日志(区分「FocusRequester 未挂载」vs 静默拒焦);②根节点 onPreviewKeyEvent 按键日志(key/类型/焦点三态)+根级自动纠焦——按键到达且合法落点无焦点时强制 requestFocus 拉回播放全部;③下轮日志可判定:按键是否到达本屏、requestFocus 抛不抛、纠焦后聚焦是否生效 | 实施中(代码完成待云编译+真机) |
| P11-72e | TV 播放列表详情页焦点真根因修复:onFocusChanged 顺序——官方文档「onFocusChanged 只监听它之后第一个 focusTarget」,本屏 playAll/视频行反写成 `.focusable().onFocusChanged{}`=绑定不到本节点,焦点回调全瞎、聚焦状态永远 false、高亮永不亮;P11-52「OK 可播但无高亮」与 72b/c/d「节点零回调/简介吃焦点/静默拒焦」全系同一监测盲区的假象,焦点其实一直成功落在播放全部(GAINED 于循环中触发可证)。改动:playAll+行 onFocusChanged/onPreviewKeyEvent 移到 focusable 前(对齐全库规范),诊断日志保留待真机闭环 | 实施中(代码完成待云编译+真机) |
| P11-47 | TV 端两次返回退出彻底退出不留后台:AppShell 退出 BackHandler 由 finish() 改 finishAffinity()+killProcess+exitProcess(0),进程不再驻留缓存后台 | Done(代码完成待云编译+真机) |
| P11-77 | TVBox 搜索 spike(内置白名单零配置):可行性研究结论(实测 3 份真实 config)——TVBox 站点 70% csp_jar / 22% drpy2 JS 需执行第三方代码(排除),type 0/1 纯 MacCMS 采集站 REST API 零代码执行可用(`ac=detail&wd=` 一步直出含 vod_play_url 的结果);config 只是地址簿,砍掉,内置量子/非凡/暴风/极速/索尼等配置重合白名单 6-8 站。TvboxRepository 并行扇出搜索(6 并发、单站 8s 超时、失败静默),同名同年份跨站合并=多线路卡(容灾对齐 IPTV 镜像心智);TV/移动 SearchScreen source toggle 加第三态「影视库」;播放走直链 m3u8;不执行任何 jar/js,零默认 config | 实施中(代码完成待云编译) |
| P11-78 | 移动端播放器横屏底栏对齐 B 站官方(用户选定范围 A,只动全屏态、竖屏不动):控制行重排为 ⏸/⏭下一集(有下一项才显示,computeNextRequest 记忆化防 500ms tick 重算)/弹幕开关/弹幕设置/弹幕胶囊输入(BiliStyle BasicTextField,占据中部弹性空间)/选集(文字按钮→底部弹层,metadata.pages>1 才显示,与简介 Tab 共用 selectEpisode)/倍速(文字按钮→菜单,非 1.0x 显当前倍速)/画质文字按钮(显示实际播放档,抽 PlaybackQualityMenu 与竖屏 HD 图标共用);时间合并单处显示在进度条上方;发送弹幕动作抽 sendDanmakuAction 两态共用;全屏退出按钮保留行尾(显式退出入口);新图标 ic_player_next + 6 locale 字符串(player_control_next_episode/player_speed_label/player_control_play/player_control_pause) | 实施中(代码完成待云编译) |
| P11-79 | B 站搜索新增番剧分类(TV+移动):类型循环按钮 视频→番剧→UP主(B站)/视频⇄频道(YouTube);SearchVideoRepository.searchBangumi(search_type=media_bangumi,WBI 签名+无签名 fallback);结果卡复用 VideoSummary(新增 seasonId 字段,bvid=ss{season_id} 去重 key,声优占 UP 主位),点击壳层拦截 seasonId>0 → PgcSeasonScreen 季详情(复用现成 PGC 播放链路),非视频类型隐藏排序 chip,切源时番剧类型回退视频,6 locale 字符串 search_type_bangumi/search_empty_bangumi | 实施中(代码完成待云编译) |
| P11-79b | 移动端搜索对齐官方 UI:类型行改三 tab 并排(综合/番剧/UP主,YouTube 综合/频道,选中粉色+下划线)+ 行尾筛选漏斗 → 底部弹层(排序方式/发布时间(最近一天/一周/半年)/内容时长(0-10/10-30/30-60/60+)),searchVideos 接口加 duration/pubtime_begin_s/pubtime_end_s 参数;TV 保持循环按钮不动 | 实施中(代码完成待云编译) |
| P11-80 | 番剧详情/选集面板对齐官方(移动端):选集改两列紧凑卡(第N话+标题+会员角标+当前集粉色高亮+顶部进度条,详情页+播放器选集弹层)、选集标题行显更新状态(newEpDesc)、>50集话数分组快捷跳(50/组,1051-1100式)、头部数据行(播放/追番/弹幕,stat)+季badge;PgcEpisode.badge/PgcSeason.stat+badge/PlaybackEpisode.badge+indexLabel 新字段 | 实施中(代码完成待云编译) |
| P11-80b | 竖屏播放器简介 tab 选集同样对齐官方:TextButton 纵列改与横屏选集弹层同款两列紧凑卡(EpisodeSheetCard 复用) | 实施中(代码完成待云编译) |
| P11-81 | TVBox 配置化(删内嵌源):删除 TvboxBuiltInSites 5 站白名单,影视库源改为用户设置里填 TVBox 配置 URL → 拉取解析 sites(kotlinx 1.9.0 allowComments 原生吃 JSONC 注释,禁按行截断),过滤 type 0/1 且 searchable≠0;TvboxRepository 注入 client+appSettingsStore、Mutex+TTL 5min 缓存、sourceStatus StateFlow(NotConfigured/Failed/Ready)、validateConfig 保存校验报站数;TV SettingsTvboxDialog+移动 MobileTvboxSection 镜像 IPTV 模式;搜索空态分流引导;manifest 开 usesCleartextTraffic(采集站 m3u8 常 http,播放走 DefaultHttpDataSource 不受 OkHttp Platform 补丁影响) | Done(代码完成,CI 绿,待真机) |
| P11-82 | WebDAV 备份/还原纳入 TVBox 配置:新增 WebDavBackupItem.Tvbox,备份 tvboxConfigUrl 到 bilitv/tvbox_config.json,还原写回 setTvboxConfigUrl(缺文件/解析失败跳过);TV/移动选择弹窗还原列表加 Tvbox 项;strings 4 locale | Done(代码完成,CI 绿,待真机) |
| P11-83 | 红果短剧内置源(新第四搜索源,零配置):spike 实锤 hongguoduanju.com 网页端全链路免签名——搜索 `/search/{kw}?__loader=search_(keyword)/page` 出纯 JSON(10 条/次,SSR 页 254KB 对比 15KB,含 series_id/标题/封面/vid_list 一步直出),播放页 `/player/{series}/{vid}` SSR 内嵌 `main_url` 明文 MP4 直链(qznovelvod CDN,裸请求 200+Accept-Ranges,无 DRM/无签名;第三方解析接口生态已全灭:cenguigui api-v2 无 A 记录/mov 返回占位文本,故选自持网页解析)。**关键限制:网页匿名端每部剧只放开前 3 集**(实测 20/20 部剧 accessible_episode_cnt=3,详情页 vid_list 虽全量但第 4 集起播放页全 404,搜索 loader 的 vid_list=可播集全集;角标按可看集数改写,登录解锁不在 v1 范围)。HongguoRepository 搜索 → VideoSummary(source=SourceHongguo,bvid=hongguo:{series_id},单线路 TvboxLine 分集=播放页 URL);PlaybackRequest.isTvbox 扩容含 hongguo(线路=清晰度面板、选集=线路内,播放 UI 全套复用);TvboxRepository.resolveLineUrl 红果分支提取 main_url(缓存同 m3u8 懒解析);播放走 ProgressiveMediaSource 分支(dummy 轨 segmentBase=null,remoteHls 置 null 防 isHlsManifest 截胡),裸头;TV 源按钮循环 B站→YouTube→TVBox→红果,移动端加第 4 chip;strings 4 locale | Done(代码完成,CI 绿,待真机) |
| P11-84 | 番剧播放记录并入历史 tab:历史接口本就返回 business=pgc 条目但 fromHistory 只读 history.bvid(pgc 时为空)被静默丢弃——按 BV HistoryData.kt + bilibili-API-collect 补解析:history.oid=该集 avid、history.epid=剧集 id、顶层 kid=季 id、cid=该集 cid;VideoSummary 新增 epId 字段,bvid 承载 "ep{epId}" 作网格 key(移动端 LazyGrid 按 bvid 做 key,空串多条相撞);toPlaybackRequest 加 epId>0 分支走 PGC playurl(ep_id+cid,bvid 置空——此时是网格 key 非稿件号),startPositionMs 门控同 UGC(看完重播/续播);heartbeat reportProgress 放宽 avid/bvid 二选一(BV sendHeartbeat 同语义,pgc 无 bvid 用 aid);PGC 季类型历史条目不带 → PlaybackVideoMetadata 新增 seasonSubType(season.type),播放器 withResolvedMetadata+TV launch effect 回填 activeRequest.subType 供 sub_type 上报;durationText 改 seasonId>0 且 duration<=0 才隐藏(番剧历史卡带该集真实时长);TV/移动历史 UI 零改动(条目自然混入既有网格) | 实施中(代码完成待云编译) |
| P11-85 | TV 设置页 TVBox 行焦点跳顶修复(SettingsItemTvbox=41 与 Account 撞号,settingsItemToLazyIndex when 先命中 Account→0 致 TVBox 分支永不可达,修 Tvbox=42)+ SABR 续播黑屏治理(09-07 真机三案:历史续播位置冻结永不 READY 重试 8+ 轮全挂;修:看门狗重试耗尽后「深度重试」evict 会话+续播点前推 10s 每视频一次,DefaultSabrChunkSource 首段裁剪诊断行取证) | Done(代码完成,待云编译) |
| P11-85b | SABR 续播黑屏根因修复:buildBufferedRanges 上报 {start:0,dur:0} 垃圾(visionOS MEDIA_HEADER.startMs/durationMs 恒 0),续播 playerTimeMs>0 时服务端回落判定被带偏 → 位置冻结;修:init 解出的段表时间网格回喂 fetcher,上报真实 startMs/durMs,无网格零风险回退 | Done(代码完成,待云编译) |
| P11-87 | TV 启动初始焦点死区修复(RecommendScreen 初始焦点兜底:Failed→重试按钮、Loading/Empty→分区 tab,Success 由网格首卡 effect 接管;FeedStatusScreen 可选 actionFocusRequester)+ 推荐预加载拆出 isLoggedIn key(LaunchedEffect(Unit),登录态翻转不再取消预拉;动态预加载保持原 key)+ loader 诊断日志(BiliMT:Home) | Done(代码完成,待云编译) |
| P11-88 | 应用图标重绘「哔哩MT」:5 版设计稿(A=BV同款白底粉机/B=官方风粉底白机/C=现款爆点渐变迭代/D=夜间深色/E=极简字形屏内MT)→用户选 E 去文字+精修(球状天线/机身投影/斜向渐变+高光/双层渐变屏/MT 100 字重800)→headless Edge 切 mipmap 五密度+adaptive 前景五密度,adaptive 背景改渐变 drawable | Done(代码完成,待真机看桌面) |
...
| P11-101 | WEB-DASH 兜底:Step 0 撤 HLS 兜底(media3 createFallbackOptions FATAL+段 403)+ Phase 1 判别探针 probeWebDashChain(WEB /player 带 token → dashManifestUrl 参数 → n/s decipher verdict → +pot GET 200);Phase 2 修订:真机实测 dashManifestUrl ABSENT+SABR-only 剥直链但 serverAbrStreamingUrl 在 → 探针转测 WEB SABR 变体;Phase 2b:打包 yt-dlp solver(meriyah+astring+core,WebView 内)+ YoutubeSolverDecipherer(n/s decipher);Phase 2c:buildWebSabrFallback 接为判死分支正式兜底(r1921 探针全链通:transformed → init POST MEDIA ok) | Phase 2c(代码完成,待云编译+真机) |
...
| P11-115 | /player cookie/visitor 桌面页会话配对(消除混搭身份;postJson 加 override 参数) | Done(代码完成,待云编译+真机) |
| P11-114 | 播放优先级设置补全:三档 SABR/DASH/WEB-SABR(枚举+resolver webSabrFirst 分支+两端 UI+4 语言) | Done(代码完成,待云编译+真机) |
| P11-116 | WEB-SABR pot-less 判别实验:会话 `streamerContext.poToken` 置空,判别「服务端拒绝我们的 token 内容」vs「WEB 会话其他维度」 | Done(真机判读:**仍 nag → token 洗清**;带 token(r1949)与 pot-less(r1950)对同一 rn 的响应**逐字节一致**,`status=1` 全程 0 次。详见 docs/youtube-web-sabr.md §3) |
| P11-117 | WEB-SABR 对齐修复(FreeTube 审计后重排):①埋点(`/att/get` 进 diag 门 + diag 加 `ua=`/`desktopVis=`/`jarVis=` + sabrUrl `c`/`n` dump + jsUrl 打内容)②`postJson` 加 `uaOverride` —— **桌面 UA 首次真正上线**(此前无入口)③`/att/get` 与挑战同源(FreeTube 形态:body 带 context 无 eacrToken + `X-Goog-Visitor-Id`/`X-Youtube-Client-Version`/`X-Youtube-Client-Name` + override)④`postPlayer` 加 `forceOkHttp`,让 /player 走唯一真带 Cookie/UA 的传输 | 实施中(计划见 docs/youtube-web-sabr.md §6;判据 G0→G1→G2,停止条件 S1/S3) |
...
| P11-122 | 修「WEB-SABR 会话全部在首帧后 0.14-1.7s 被自己的看门狗杀掉」:两个看门狗计时器跨起播期累加,首帧渲染那一轮同时触发「25s 档→8s 档」阈值切换与 `bufferingPosAdvanced` 置真 → 「刚播起来」立刻被判成「挂死」→ 重载死循环(`PlayerScreen` 轮询检测 `frameRendered` false→true 跳变,那一刻清零计时器) | Done(真机验证通过:r1985,`logs_live_20260919_230913.log` 全日志看门狗零开火;含 20.5s 慢起播与 0x0 重建 blip 自愈两处实证) |
| P11-123 | TV 播放器底栏直连「画质」「字幕」入口(**先做 YouTube**):`PlayerControl` 新增 `Quality`/`Subtitle`(枚举位置在 `Related` 之后 → 控制行顺序 [选集][UP][相关][画质][字幕][互动…][评论][设置]) + 新增 `PlayerValueButton`(图标 + 当前值文本、宽度自适应、液态玻璃焦点态) + 画质文案从右下状态区**移入按钮**(「占原显示位」:`showQualityText = Quality !in availableControls`,状态区只剩在看/弹幕) + 字幕按钮按 `hasSubtitleChoice(info)` 条件出现(与 Main 面板同判据) | 实施中 |
| P11-124 | B站 播放器 UI 对齐官方客户端(云视听小电视):顶部区块重做(标题 2 行 + 元信息单行「UP主 · 播放 · 弹幕 · 点赞 · 收藏 · 投币 · 发布于」+ 动作行「点赞/收藏/投币/稍后再看/评论」)+ 底部控制条重排(倍速文案 `1.0x` → UP主页/刷新/字幕/弹幕开关/播放列表/相关视频/画质/设置;裸图标 + 白描边焦点环,撤掉液态玻璃方块)+ 焦点模型扩成三级(动作行 → 进度条 → 控制行)+ 删掉 B站 右下状态文案 + 新增「刷新」入口 | 实施中(代码完成待云编译+真机) |
| P11-125 | harvest 采集腿取证 + 空壳页丢实例重建(09-19 真机 `logs_live_20260919_211811` 整场 0 捕获的响应):①`postPlayer` rethrow 前落**耗时 + 异常类名/message/栈**(真机三次 WEB-SABR `/player` 在 identity 日志后 1ms 内失败却零日志,分不清「瞬时抛错」与「网络超时」);②`buildWebSabrFallback` 两处调用点补 `.onFailure` 落证(整条链此前只剩一句 `abort`);③harvest 侧主文档/子资源错误**无条件**累计 + fail-fast/timeout 时汇总打 `mainFrameErr=`/`httpErr=`/`last=`(旧实现只记 URL 含 youtube/googlevideo,主文档走了别的 host 就漏);④PAGE diag 补 `dl=`(documentElement.outerHTML 长度)/`rs=`/`ytcfg=`,新增 `DOC_LEN_JS` 走 evaluateJavascript **同步回传**长度(空壳判定不能等 console);⑤空壳页判据扩一条「onPageFinished **已触发**但主文档 <20KB」(旧逻辑只判「未触发」,「完成了但空」会干耗满 30s);⑥`invalidateWebView()` 丢弃实例 + 下次 harvest 重建(alpha.61 长期存活的前提是实例健康;重建后仍空壳 ⇒ 判死实例层假设,直指网络/风控层) | 实施中(代码完成待云编译+真机) |
| P11-126 | 修「30s 起播预算杀掉整条 launch,连兜底会话也白建」(P11-125 真机判读的直接遗留项):①**预算按交付档给**(新 `YoutubeLaunchBudget` 单一真源):B站/影视库/IPTV/红果保持 30s、YouTube SABR/DASH 45s、YouTube WEB-SABR 优先 90s;`PlayerScreen` 按 `youtubeDeliveryPriority`(P11-126 起从 AppShell 透传)取预算,并把绝对 `deadlineMs` 透传 `getPlaybackInfo`→`resolve`(带默认值 0=不限,既有调用点零改动)。②**预算不足早退**:进 WEB-SABR 优先/兜底前先看剩余预算,低于 `MinWebSabrFirstBudgetMs`(45s)整条跳过直落 NewPipe;`harvestSessionMaterial` 两次尝试从硬编码 40s/30s 改为 `min(硬上限, 剩余-FallbackReserveMs 12s)`,剩余不足 3s 干脆不发(不再烧注定被砍的 WebView/solver)。③**预算耗尽可见**:`withTimeoutOrNull` 返 null 时打 `launch timeout after Nms (step=…)`(此前静默),超时文案从写死「起播超时 30s」改带真实秒数(4 locale)。④**harvest WebView 启动预热**:`prewarm()` 把冷启(建实例+载首页,真机 10.9s)移出起播;`AppContainer.startYoutubeHarvestPrewarm()` 启动后延迟 8s,只在 WEB-SABR 优先档做;`ensureWebView` 加 `webViewMutex` 串行化创建。⑤**空壳页取证探针**:`FORENSIC_JS` + `runForensicProbe`(包 `NonCancellable`,否则取消态下证据打不出来)拿四组数据——文档内容 / `performance` 导航 `encodedBodySize`(判「服务端只发 39 字节」vs「发了 1.4MB 但文档空」)/ `navigator.userAgentData` 与 UA 是否自相矛盾 / 页内同源 fetch 能力,外加 cookie jar 的 SOCS/CONSENT 与主文档请求头。社区线索(Sec-CH-UA 矛盾 / consent cookie / `; wv)`)见 docs §3.6,**本轮只取证不改行为** | 实施中(云编译绿 25d587d4;待真机) |
| P11-127 | WEB-SABR 转**全移动自洽身份**(Stage 1):①采集 WebView UA 桌面→移动(+补 m.youtube.com cookie seed + 身份自证探针);②`/player` 撤掉桌面 context/cookie/visitor/UA 四件套、SABR 会话恒用 `sabrClientInfo()`(clientName=1 + Android osName)+ 移动 UA;③WEB-SABR 的 token 换**移动 minter**(`PoTokenWebView`/LibreTube 链),不再用「桌面 watch 页取挑战 + att/get 桌面 ctx」那条链(移动 UA 下 watch 页 302 且无 `ytAtN`,该链结构性不可用);④删 `webDesktopSabrClientInfo` | 实施中(代码完成待云编译+真机;判据 G0-G5/M1-M3 + kill K1-K4 见 PROGRESS) |
| P11-128 | 起播档**锁高搬进选择内部**,消除「松 cap 必重建」:新增 `HeightAwareAdaptiveTrackSelectionFactory.startupLockHeight`(选择类每次评估现读),选择类初始档取锁高档 + `selected` 写入统一过锁高夹取;两端 player 撤掉 `setMinVideoSize/setMaxVideoSize`,首帧回调改调 `releaseStartupLock()` | 实施中(代码完成待云编译+真机;验收:日志 `init trackType=… trackSelLen=N` 只在起播出现一次、`updateTrackSelection: sel X → sel Y` 接管后续换档、起播后 `bufS` 不再归零) |
| P11-129 | 清晰度菜单对齐 B站/LibreTube:**只列分辨率**——SABR 路径按 height 去重(同 height 多 codec/帧率变体合并),标签只留 `"${h}p"`;代表轨由**「解码器」设置**(`PlaybackCodecPreference`)决定(手动选中 → codec 偏好 → 码率),换设置仍停在同一分辨率 | 实施中(代码完成待云编译+真机) |
| P11-130 | 升档预加载:chunk source 在「下一档可负担 + 缓冲≥20s」时把候选档报给 fetcher;fetcher 把它写进请求 `preferredVideoFormatIds`、**不报它的 bufferedRange**(报"没有"=请推数据)、并在清理非当前格式时保住它 → 服务端把候选档 init(+段)推回来缓存,切档那刻命中缓存不现拉 | **未闭环(真机证伪:候选档未被服务端采纳,bufS/升档耗时与改动前同档;见 PROGRESS)** |
| P11-131 | 设置重构:**「YouTube 设置」可折叠一级分组**(两端)+ **解码器/默认倍速各拆成 B站·YouTube 两份独立值**:①`AppSettings` 加 `youtubePlaybackCodecPreference`/`youtubeDefaultSpeed`(键 `youtube_playback_codec_preference`/`youtube_default_speed`),**不写迁移**——读取端 `preferences[新] ?: preferences[旧]` 回退(沿用 `low_spec_mode`→`visual_performance_mode` 既有范式)⇒ 存量用户零变化、首次改动后两端独立;②`PlaybackRepository.getPlaybackInfo` **末尾**加 `youtubeCodecPreference = codecPreference`(位置实参调用点 `DownloadUrlResolver`/`MobileApp` 零改动),`isYoutube` 分支改用;B站 路径(fnval/轨道过滤/comparator/缓存键)仍走共享值;③倍速:TV `PlayerScreen` 起播按 `request.isYoutube` 选值(`remember` 保持无 key,不打断播放中手改);**移动端播放器仍写死 1f(本次不修,见 P11-132)**;④TV 首个可折叠分组:新增可聚焦 `SettingsCollapsibleSectionTitle`,`settingsItemToLazyIndex` 拆成 `expandedItemToLazyIndex` + 包装(组内 9 行按 `SettingsYoutubeGroupItems` 集合判 -1,组后行整体 `-size` 上移,`YoutubeGroupHeaderLazyIndex=26`),新常量 47/48/49 且加常量唯一性运行时守卫,**默认展开**(默认折叠若组头焦点坏 = YouTube 设置全不可达);⑤移动端并入同一分组(`MobileYoutubeSabrSection` → `MobileYoutubeSettingsSection`,默认折叠);⑥文案 6 locale 加 7 键;⑦`WebDavBackupService` 不动(只备份 Piped 配置) | 云编译绿(7c479824);待真机验收 |
| P11-132 | 移动端播放器读「默认倍速」:`MobilePlayerScreen.kt:401` 把起播倍速写死 `1f`,**从未消费 `defaultPlaybackSpeed`** ⇒ 移动端该设置一直失效。修=按内容源读 B站/YouTube 两份值 | 待办(P11-131 已把两份值备好) |
| P11-133 | **YouTube 解码器独立值域(含 VP9)+ ABR 梯子跟随**:①新枚举 `YoutubeCodecPreference(auto/vp9/av1/h264/h265)`,`codecKey` 映射到既有 `codecRank`/`codecKeyOfItag` 的族键(vp9/av01/avc/hevc);key 字符串**沿用旧共享键**只新增 `vp9` ⇒ 从 `playback_codec_preference` 回退读取时存量用户的 H264/H265/AV1 原样映射,不会退化成 Auto。②`AppSettings`/`AppSettingsStore`/`PlaybackRepository`/两端 player/shell 全线换型;`isSupportedBy(capability)` 在 repository 与两端 shell 拦一道(解不了→Auto,防代表轨钉在解不出的 codec 上全档零帧)。③`codecRank` 改「选中族置顶 + Auto 顺序兜底」,Auto 行为与改前逐字节等价。④**粘性梯子锚点跟随**(用户拍板):`HeightAwareAdaptiveTrackSelection` 加 `preferredCodecFamilyProvider` + `codecFamilyOf(Format.codecs)`(avc/av01 同为 video/mp4,只能靠 codecs 串区分),非空且**本组确有该族变体**时锚点从「全组顶档 codec」换成用户选的族——粘性照旧(整场单 codec 不跨族重建),Auto 走历史 mime 判据不变;组内无该族则退回 Auto 锚点(否则 `bestIndexOf` 只剩码率兜底,同 height 各挑各的 codec 反而制造跨族换解码器)。⑤两端设置页该行改用新枚举(移动端不过滤、TV 按 `isSupportedBy` 过滤,各自沿用本平台既有做法);⑥文案 6 locale 加 5 个选项键 + 重写描述(B站 那句「自动优先 AV1 > H.265 > H.264」与 YouTube 无关,不再复用) | 实施中(代码完成待云编译+真机;**H.265 项已按真机判读移除**——全部日志零 HEVC 轨) |
| P11-134 | **移动端补回 stall 看门狗 + SABR POST 整调用上限**:①移动端 `MobilePlayerScreen` 的 500ms 轮询循环里补回 TV 那套看门狗(位置冻结 `StallThresholdMs=8s`/未出首帧 `StartupStallThresholdMs=25s`;视频冻结 `VideoFreezeThresholdMs=12s`——BUFFERING 但位置仍前进=音频时钟活着、视频渲染挂死),新增 `frameRendered` 状态作档位开关(loadRequest 清零、onRenderedFirstFrame 置位+清零计时器,防「续播慢起播被自己判死」的 P11-122 死循环),起播期 stall 额外 evict SABR 会话换新(P11-95);②`SabrMediaFetcher` 的 SABR POST 加 `SabrCallTimeoutMs=40_000L` 整调用上限(playback client 是 `callTimeout(0)`+per-read 15s ⇒ 慢滴永不超时) | 实施中(代码完成待云编译+真机) |
| P11-180 | **B站图文动态移植 · 切片1:数据层(纯诊断,不碰 UI)** —— 关注流看不到图文不是接口不给:① `getDynamicFeed(type="video")` 服务端只回视频投稿;② `fromDynamicItem` 只收 `MAJOR_TYPE_ARCHIVE`/`live_rcmd`,图文/纯文字/转发/专栏一律 drop;③ `VideoSummary` 是视频形态,正文与多图无处安放。本轮只做数据层:新增 `DynamicImage` + `VideoSummary.dynamicKind/dynamicText/dynamicImages/dynamicTextHasMore`,`fromDynamicItem` 增 `DYNAMIC_TYPE_DRAW` 分支(**两条载荷分支兜底**:文案 `desc.text ?: major.opus.summary.text`、图片 `major.draw.items ?: major.opus.pics`;宽高在 draw.items 是**字符串**、opus.pics 是**数字**,统一走 `BiliNumberParser`),仓库层加每页类型占比打点 `dynamic feed type=… items=… returned=… draw=… drawWithText=… kinds={…}`。**UI/行为零变化**:图文仍被 `bvid.isNotBlank()` 过滤(卡片还没做),TV「动态·全部」与移动端动态 tab 表现不变。切片2 = 图文卡 + 九宫格 + 展开/收起 + 点图看大图(看图器自研最小集);切片3 = 详情页(opus 全文 + 计数)+ 动态评论 `type=11`(楼中楼先不做)。点击行为对齐 BV:点卡片进详情页、点图片看大图 | 实施中(代码完成待云编译) |
| P11-181 | **B站图文动态移植 · 切片2:图文卡 + 移动端动态 tab 放开图文**(切片1 已让数据层能解析图文,本切片把它显示出来) —— ① 取数口径:移动端动态 tab 由 `type=video` 切 `type=all` + `includeDraw=true`(服务端 `type=video` 时**根本不返回图文**);请求加 `features=itemOpusStyle`(不加则图文正文恒为空 —— 实测两次独立采样正文长度全 0,加了才有 231/132/115… 字;该值取 BV 的最小集合,且实测视频动态既有字段完整度不变、archive 未被挪进 opus);② 新卡片 `MobileDynamicDrawCard`:作者块 → 正文(6 行折叠 + 展开/收起)→ 九宫格(1 图 2:1 / 2 图并排 / ≥3 前 3 张 + 「+N」)→ 点赞/评论/转发计数;图片每格显式拼 CDN 尺寸后缀(九图否则拉 9 张原图)并跟随性能档(RGB_565 + 内存缓存开关);③ 列表 key/去重换 `VideoSummary.feedKey`(图文无 bvid,回退 dynId);④ 6 locale 新增展开/收起/转发计数文案。**本轮整卡与图片不响应点击**(点卡片进详情页、点图看大图分别在后续切片接);作者块仍可点进 UP 主页 | 实施中(代码完成待云编译) |
| P11-182 | **移动端动态卡对齐官方参照图**(用户提供官方关注流截图作参照:视频卡 = 作者行 + **动态正文** + 缩略图(左下「时长·播放·弹幕」覆盖)+ 标题 + 卡底互动计数行;图文卡 = 作者行 + 正文 + 图片 + 同款计数行) —— ① **视频动态此前丢了自己的文案**:`fromArchiveDynamic` 只取 `archive.title`(视频标题),UP 的 `module_dynamic.desc.text` 从未解析 ⇒ 参照图里最显眼的那段话我们完全没有。已补 `dynamicText`(desc.text,opus 兜底);② 作者行的「时间 · 播放量」改为**只留时间**,播放/弹幕挪到缩略图左下角覆盖行(`%1$s播放` / `%1$s弹幕` 复用既有字符串 + `durationText()`),对齐官方「13:08 · 4.2万播放 · 55弹幕」;③ 卡底补互动计数行(转发/评论/点赞,带 `ic_player_share`/`ic_player_comment`/`ic_player_like` 图标),仅 `dynId` 非空的动态条目显示;④ 新增共用 `DynamicActionRow`,图文卡与视频动态卡同款(图文卡原来的纯文字计数行替换掉)。改动全部落在 `feedLayout=true` 这条分支(仅动态 tab 使用),首页/搜索/频道页不受影响 | 实施中(代码完成待云编译) |
| P11-183 | **动态正文里的 B站 表情(表情包)内联渲染** —— 真机反馈「图文可以,但是表情包无法显示」:正文里自定义表情只显示成 `[池年x悠米·双生回响动态表情包_贴贴]` 这类**占位文字**。原因:B站 正文是 `rich_text_nodes[]`,话题/@/网页链接这些节点自带可读 `text`,**唯独表情不行** —— 它的 `text` 就是占位文案,真图在 `emoji.icon_url`;而我们此前只取 `desc.text`/`summary.text` 平文本。**实测**(本机 wbi 签名,8 个 UP):节点类型分布 TEXT 37 / EMOJI 12 / AT 2 / TOPIC 1 / WEB 1;表情节点形如 `{text:"[保佑]", emoji:{icon_url:"https://i0.hdslb.com/bfs/emote/….png", size:1, …}}` ⇒ ① **`emoji.size` 是档位不是像素**(样本值 1),尺寸该用固定 token;② **hdslb 的 emote URL 同样吃尺寸后缀**(原图 PNG 1989B → `@60w_60h_1c.webp` 1202B)。**修法**:模型加 `DynamicTextNode(text, emojiUrl)` + `VideoSummary.dynamicTextNodes`;映射层 `textNodesOf()` 解析 `rich_text_nodes`(仅当存在表情时才返回节点,否则退回平文本,行为与改动前等价),图文与视频动态两条分支都接;新增共用 `DynamicRichText`(Compose `InlineTextContent` 把表情内联成 `BiliSizing.DynamicEmojiSize` 的小图,其余节点拼文本),图文卡与视频动态卡的正文都改走它 | 实施中(代码完成待云编译) |
| P11-184 | **动态大图查看器**(点图文图片看大图) —— 自研最小集,不引第三方看图库:全屏 `Dialog`(同 TV 评论页范式,不必把状态提到 shell 层就能盖住底栏)+ `HorizontalPager` 左右翻页 + 双指缩放/双击放大/放大后拖动,底部页码指示。**关键取舍**:① `Modifier.transformable` 默认会吞掉单指拖动(那会抢走 Pager 翻页),改用带 `canPan` 门控的重载 —— 1x 时不接管拖动、放大后才允许拖;② 大图请求**只给宽不给高**(`@Wx.webp`),带高会追加 `_1c` 变成居中裁切,大图必须保持整图比例(新增 `buildFullImageRequest`);③ 缩放档位进 token(`BiliFocus.ImageViewerDoubleTapScale` / `ImageViewerMaxScale`);④ 点「+N」角标直接从第 4 张起开。交互:单击关闭、双击 1x⇄放大、双指缩放到上限 | 实施中(代码完成待云编译) |
| P11-185 | **动态详情页**(点图文卡正文/计数区打开) —— 全屏 `Dialog`(与查看器同范式),单条滚动列表:作者行 → **正文不折叠**(列表卡折 6 行)+ 截断提示 → 九宫格(点图开查看器)→ 互动计数 → 评论区。**评论复用**:把播放器的 `MobileCommentList` 参数化成 `CommentTarget`(Video/Dynamic),`oid=dynId&type=11` 走同一个 `/x/v2/reply`;`SortChip`/`CommentItem`/`CommentListFooter`/两个 loader 放开为 internal 直接复用(零重复);评论列表参数化后播放器调用点改为传 target(PGC → null 显示占位)。**九宫格抽共享件** `DynamicImageGrid.kt`(列表卡与详情页同一套渲染)。**详情数据不发新请求**:实测 `x/polymer/web-dynamic/v1/detail` 离线回 `-352`,直接用列表已解析的那条;超长正文可能被服务端截断,`has_more` 时给一行提示 | 实施中(代码完成待云编译) |
