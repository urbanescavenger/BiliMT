...
| P11-101 | WEB-DASH 兜底:Step 0 撤 HLS 兜底(media3 createFallbackOptions FATAL+段 403)+ Phase 1 判别探针 probeWebDashChain(WEB /player 带 token → dashManifestUrl 参数 → n/s decipher verdict → +pot GET 200);Phase 2 修订:真机实测 dashManifestUrl ABSENT+SABR-only 剥直链但 serverAbrStreamingUrl 在 → 探针转测 WEB SABR 变体(③' parseSabrData+n-decrypt verdict,④' WEB SABR 会话+init POST 全链判据);Phase 3 待 go 后接 buildSabrSessionFromWebPlayer | Phase 2 修订(代码完成,待云编译+真机探针) |
