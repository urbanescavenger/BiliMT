...
| P11-126 | 修「30s 起播预算杀掉整条 launch,连兜底会话也白建」(P11-125 真机判读的直接遗留项):`PlayerScreen.kt:1968` 的 `withTimeoutOrNull(LaunchTimeoutMs=30_000)` 包住整条起播(含 resolve),而 WEB-SABR 优先档的固定开销已达 ~21-28s(PO token 铸造 ~9s + player jsUrl/signatureTimestamp ~4s + harvest WebView 冷启首页 4-11s),watch 页还没开始加载预算就没了 ⇒ resolve 后半段整体被取消:`postPlayer` 0ms 抛 `TimeoutCancellationException` → WEB-SABR abort,随后成功建好的 NewPipe SABR 兜底会话**也因协程已死而无人消费**(日志 `launch step:` 只到 `playurl`,无 `cdn`/`prepare`)⇒ 用户黑屏 ~99s 直至手动退出。候选方案(待用户选定):**F4** 预算耗尽必须可见(超时时打明确日志 + 触发重试/兜底,现在静默);**F3** 进 harvest 前先看剩余预算,不够就不做 WEB-SABR 优先、直接落 NewPipe(止血:至少兜底能落地);**F2** harvest WebView 冷启(建实例 + 首页)移出起播路径,app 启动后后台预热(本次 10.9s 全花在这);**F1** 按源/按档给预算(WEB-SABR 优先档 ≥60s)而非 UI 层一个固定 30s | Pending(待用户选定方案) |
