...
| P11-14 | YouTube 高清播放(Tier 1:`s` 解密 + adaptive 首选 + DASH 播放 + 硬件过滤 + 多档清晰度;Tier 2:PO token) | 实施中(2.0.10-alpha.3 诊断:token 在 body 但判无效,skipPrivacyBuffer/GenerateIT 端点均非根因,已加带/不带 token 对比诊断,待真机) | 方案落盘 `docs/youtube-hd-playback.md`。**实测结论**:无 PO token 时 YouTube 在所有客户端剥掉 adaptive 高清 url(只剩 progressive 360p),ANDROID 客户端也剥——高清唯一前置是 PO token。Tier 1 已落地:`s` 解密(`YoutubeSDecryptor`)+ adaptive 首选 + 合并 WEB+ANDROID 候选 + `pickVideo` 最大化分辨率 + `CodecCapabilityProbe` 硬件过滤 + `initRange`/`indexRange` 填 segmentBase 走 DASH + 多档清晰度。Tier 2 已落地:打包 bgutils-js(MIT)进 `assets/youtube/bgutils.js` + `YoutubeBotGuard` 完整流程(challenge `/api/jnn/v1/Create` requestKey=`O43z0dpjhgX20SCx4KAo` → descramble → interpreter 加载 → snapshot → GenerateIT → mint → 视频 ID 绑定 PO token)+ resolver 注入 `/player` `serviceIntegrityDimensions.poToken`。**PO token 真机调试已修掉 4 个障碍**(详见 `docs/youtube-hd-playback.md` §6.7):① WebView 重建导致 `__runSnapshot` 丢失 → eval 抛错检测重建 + 验证重载;② WebView shell 未就绪就 eval → `onPageFinished` 置位 `shellReady` + `awaitShellReady`;③ pollState 双重 JSON 编码解析;④ GenerateIT 响应 token 在 index 3。**已跑通**:challenge → interpreter → snapshot(`botguardResponse`)→ GenerateIT(`[null,43200,null,"<token>"]`)。**待真机验证(alpha.12 起)**:GenerateIT 解析修复后 → mint(WebPoMinter)→ `PO token minted`;若 mint 报错看 `PO token JS error`(可能 webPoSignalOutput 或 contentBinding `c` 占位)。**其余待真机**:DASH 出帧、4K 无硬解回退 |
