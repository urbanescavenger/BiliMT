...
| P11-14 | YouTube 高清播放(Tier 1:`s` 解密 + adaptive 首选 + DASH 播放 + 硬件过滤 + 多档清晰度;Tier 2:PO token) | Pending | 方案/可行性落盘 `docs/youtube-hd-playback.md` + PLAN 加 P11-14 行。现状:YouTube 最高只到 720p(Case B 优先 progressive itag 18/22);`adaptiveFormats` 高清 itag 已解析但选不到,`signatureCipherUrl` 未做 `s` 解密(403)。Tier 1 = 复用 `YoutubeJsExecutor` 做 `s` 解密 + `pickVideo` adaptive 首选 + `CodecCapabilityProbe` 硬件过滤 + `parseFormat` 补 `initRange`/`indexRange` 填 segmentBase 走 DASH。**运行时待真机**:`s` 解密(base.js 常变)、DASH 出帧、4K 无硬解回退 |
