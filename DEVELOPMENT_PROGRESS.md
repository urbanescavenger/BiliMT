...
| P11-14 | YouTube 高清播放(Tier 1:`s` 解密 + adaptive 首选 + DASH 播放 + 硬件过滤 + 多档清晰度;Tier 2:PO token) | 实施中(2.0.10-alpha.4 诊断:带/不带 token 响应完全一样都剥空 → token 完全无效;已补 challenge context 浏览器指纹字段,待真机) | 方案落盘 `docs/youtube-hd-playback.md`。**实测结论**:无 PO token 时 YouTube 在所有客户端剥掉 adaptive 高清 url(只剩 progressive 360p),ANDROID 客户端也剥——高清唯一前置是 PO token。Tier 1 已落地:`s` 解密(`YoutubeSDecryptor`)+ adaptive 首选 + 合并 WEB+ANDROID 候选 + `pickVideo` 最大化分辨率 + `CodecCapabilityProbe` 硬件过滤 + `initRange`/`indexRange` 填 segmentBase 走 DASH + 多档清晰度。Tier 2 已落地:打包 bgutils-js(MIT)进 `assets/youtube/bgutils.js` + `YoutubeBotGuard` 完整流程(challenge `/att/get` → descramble → interpreter 加载 → snapshot → GenerateIT → mint → 视频 ID 绑定 PO token)+ resolver 注入 `/player` `serviceIntegrityDimensions.poToken`。**PO token 真机调试已修掉 4 个障碍**(详见 `docs/youtube-hd-playback.md` §6.7):① WebView 重建导致 `__runSnapshot` 丢失 → eval 抛错检测重建 + 验证重载;② WebView shell 未就绪就 eval → `onPageFinished` 置位 `shellReady` + `awaitShellReady`;③ pollState 双重 JSON 编码解析;④ GenerateIT 响应 token 在 index 3。**已跑通**:challenge → interpreter → snapshot(`botguardResponse`)→ GenerateIT → mint → `PO token minted`(128B)。**alpha.4 关键诊断**:带/不带 token 的 WEB /player 响应**完全一样**(`rawAdaptive=30 firstUrl=EMPTY`)→ token 在 body 里但被 YouTube 判无效。对照 youtubei.js `#buildContext` 发现 `buildContext(Client.WEB)` **缺浏览器指纹字段**(osName/osVersion/browserName/browserVersion/deviceMake/deviceModel/memoryTotalKbytes/timeZone/deviceExperimentId/rolloutToken)。**本次修改**:`fetchRealSessionData` 扩展解析 device_info[11][12][17][18][79][86][87][103][107] 存入 `RealSessionData`;`buildContext(Client.WEB)` 补指纹字段;`fetchBotGuardChallenge` 加 challenge context 诊断 log。**待真机验证**:补指纹后 token 是否变有效(adaptive>0);若仍 0,查 contentBinding 是否需带 `c` 字段。**其余待真机**:DASH 出帧、4K 无硬解回退 |
