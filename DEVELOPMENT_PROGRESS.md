...
| P11-15 | WebDAV 备份/还原 YouTube 关注频道 | 实施中(代码改完待云编译) | 新增 `core/webdav/` 包:`WebDavConfigStore`(DataStore 明文存 URL/账号/密码)、`WebDavRepository`(OkHttp PUT/GET/MKCOL + `Credentials.basic` Basic auth,复用 `downloadHttpClient` 长超时)、`WebDavBackupService`(序列化 `List<YoutubeChannel>` 成单个 JSON,上传到 `{url}/bilitv/youtube_channels.json`,还原下载解析写回 `YoutubeChannelStore`);`AppContainer` 注入三组件;TV `SettingsScreen` 加「WebDAV 备份」行 + `SettingsWebDavColumn` 右面板(内嵌 D-pad 键盘,URL/账号/密码字段切换 + 备份/还原按钮,懒索引重编号);移动 `MobileSettingsScreen` 加 `MobileWebDavSection`(三个 `OutlinedTextField` + 备份/还原按钮);`AppShell`/`SettingsActivity`/`MainActivity` 接线。**运行时待真机**:WebDAV 服务器连通、备份文件生成、还原频道恢复 |
