...
**切片进度**:切片1(数据层,纯诊断,UI 零变化)= P11-180 实施中 → 切片2(图文卡 + 九宫格 + 展开/收起 + 点图看图器)→ 切片3(详情页 + 动态评论 `type=11`)。

**切片1 实测补充**:同一条带正文的动态,不带 `features` 只取到 **0 字正文 + 1 图**,带上 `features=itemOpusStyle` 取到 **231 字正文 + 1 图** ⇒ 正文只存在于 `opus.summary`,**卡片要出正文则请求很可能必须带 `features`**。切片1 用 `drawWithText` 打点确认;加 `features` 时需同时核对不会改变 archive / live_rcmd 的既有解析。
