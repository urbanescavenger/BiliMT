...
**§6.7 row 42(alpha.18 真机——传输头已带仍 403;alpha.19 定位根因:sabrUrl 未 decipher)**:alpha.18 真机(17:35)日志:`fetch rn=0 isInit=true stream=VIDEO body=1641B` → `HTTP 403 headers=[Server=gvs 1.0; Content-Type=application/vnd.yt-ump; Content-Length=0; Access-Control-Allow-Origin=https://www.youtube.com; Access-Control-Allow-Credentials=true; ...]`。**关键解读**:①`Server=gvs 1.0` = Google Video Server 真实 SABR 后端(请求穿透到应用层,URL 签名/edge 通过);②`Content-Type=application/vnd.yt-ump` + `Content-Length=0` = 服务端**识别**为 SABR/UMP 请求但**应用层拒签**(非 CDN edge 403,非 IP);③传输头已确带(`ua=Mozilla/5.0 (Linux; Android 13; Pixel 7) cookie=556B visitor=520B`)但未解 403 → 传输头非根因。**核对 googlevideo `examples/downloader/utils/sabr-stream-factory.ts`(工作参考)**发现决定性差异:`serverAbrStreamingUrl = await innertube.session.player?.decipher(playerResponse.streaming_data?.server_abr_streaming_url)`——**SABR URL 在 POST 前必须经 player `decipher`(n-param transform + sig)**。我们 line 125 `val sabrUrl = streamingData.stringOrNull("serverAbrStreamingUrl")` 取的是**未 decipher 的原始 URL**,带未解 `n` 签名参数 → googlevideo 经典 403 空体(与 `YoutubeNDecryptor` 文档「否则 googlevideo 返回 403」完全吻合)。另发现 `enabledTrackTypesBitfield` 误用 `0`(VIDEO_AND_AUDIO)应 `2`(VIDEO_ONLY)——对齐 googlevideo `EnabledTrackTypes` 枚举(`VIDEO_AND_AUDIO=0/AUDIO_ONLY=1/VIDEO_ONLY=2`),createVideoPlaybackAbrRequest 按 `currentFormat.width` 取 VIDEO_ONLY/AUDIO_ONLY。另:googlevideo 工作示例 `clientInfo` 仅送 `{clientName, clientVersion}` 两字段(非我们 rich 的 device/screen/timezone)——暂不改,先验证 decipher 是否即根因。**alpha.19 修**:①`YoutubePlaybackResolver` 探针在 `fromSabrData` 前 `decipherSabrUrl(sabrUrl, resolvePlayerJsUrl(videoId))` 复用现有 `nDecryptor.decrypt`(与 legacy DASH `resolveStreamUrl` 同款 n 解密);②`decipherSabrUrl` 同时 dump 全量 param key + `n/sig/s/pot` 存在性 + n-decrypt `applied`/`NO-CHANGE` —— alpha.18 仅截 200 字看不到 n 是否在 URL 里,此日志在 403 持续时定位是「n 未解」还是「poToken 绑定错」还是「URL 本无 n」;③`SabrClient.enabledTrackTypesBitfield` VIDEO→`2`。待真机(alpha.19)验证:403→200?(则 decipher 即根因)或 n-decrypt log 示 `NO-CHANGE`(则 base.js plasma WASM n-transform 失效,见 `youtube-plasma-wasm-n-decrypt`)或 n 不在 URL(则转查 poToken 绑定)。
