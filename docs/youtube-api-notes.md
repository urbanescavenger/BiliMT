...
- **fork 类型注意**：`info.subtitles` 返回 `List<SubtitlesStream>`（fork 改名，非上游 `SubtitleInfo`）。语言 `getLanguageTag()`（无 `getLanguageCode()`）、显示名 `getDisplayLanguageName()`、`isAutoGenerated()`；`Stream.getUrl()` 返回 `String?`（`baseUrl` 赋值需 `orEmpty()`）。NewPipe 侧固定 `replaceAll("&fmt=[^&]*","")` 后追加 `&fmt=vtt`。
