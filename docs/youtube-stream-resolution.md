...
>
- **状态(2026-08-16,alpha.82)**:**多 Representation 修音频 itag 暴露成视频轨**——alpha.81 多 Representation 生效后真机仍不能播:ExoPlayer 实际请求 **itag248(Opus 音频)**而非视频 itag → 服务端 RELOAD 死循环。根因:NewPipe 把音频 itag248 误分类进 `videoOnlyStreams`,`SabrManifest.fromSession` 把所有视频轨塞进同一 video AdaptationSet,`Representation.fromTrack` 仅按 `trackType==VIDEO` 强制建成视频 Format → 音频被当视频轨选。修复(对齐 LibreTube):① `Representation.fromTrack` 建视频 Format 加 `MimeTypes.isVideo(mimeType)` 守卫(音频 itag 建成 audio Format);② `SabrManifest.fromSession` 视频轨按 `mimeType` 分组、每个 mimeType 一个 video AdaptationSet(`groupBy { it.mimeType }`)。真机验证:播 `jNl6YkkzKxw` 应请求视频 itag(315/313)而非 itag248、无 RELOAD 死循环。清晰度选择功能暂时去掉(ExoPlayer 自动选轨)。
