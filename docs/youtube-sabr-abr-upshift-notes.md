...
## 33. 2026-09-20「降档到 144p 后再也不升档」:一次冻结连降四档 + 180s 冷却锁死下一级 (P11-135)

### 现象(真机 `logs_live_20260920_135925.log`,r2002,移动端,视频时长 527s)

播放到 pos≈63s 起:**32.8 秒零 ABR 评估**(13:57:36.057 → 13:58:08.889),水位从 31.9s 自然漏到
5.8s;随后 **5.7 秒的冻结窗口里连降四级** 720p→480p→360p→240p→144p;落 144p 后 **112 秒零升档**
直到日志结束(用户手动退出)。同期带宽 `bw` 3.0–11.3M、`sus` 4.0–21.7M、`cap` 3.9–5.0M、bufS 峰值
**34.8s**(多次 ≥30s,`canUpgrade` 达标),144p 之上任何一档的门槛都只有几百 K —— **门全开,就是不动**。
...
**② 一次冻结连降四档(本 commit 主症状)**:冻结窗口只有 **5.7s**
(13:58:08.935 `playerState=3` → 13:58:14.635 `playerState=2`),里面塞了 4 次水位急救:

| 评估时刻 | bufS | 动作 | 与上次评估 |
|---|---|---|---|
| 13:58:08.889 | 5.8s | →480p | (①的 32.8s 零数据漏下来的存量) |
...
——`lastDowngradeElapsedMs` 全场只被 `canUpgrade` 读,水位急救分支从不读它。于是同一段 5.7s 冻结被
拆成 4 个独立评估、**算了 4 次账(冻结时间在累加)**。而每次降档自身还要 1.5–3.4s 拉新档 init
(1856B 小请求 + 服务端 `NEXT_REQUEST_POLICY backoff=2000ms`),期间零可用数据 → 水位继续漏 →
下一轮又成立。5.8s 的存量买单 4 次切档,自然穿到地板。
**对照**:13:58:14.635 冻结结束后**只要不再降档**,缓冲 13:58:18 就回到 5.8s、13:58:22 回到 30.3s
——**4 秒回满**。正确行为是冻结期只降一档。
...
**A. 冻结 episode(`HeightAwareAdaptiveTrackSelection`)**:首次跌破急救阈值 → 降一档并置
`freezeEpisodeActive`;episode 内不再触发第二次水位降档;水位回到阈值以上(恢复)才清位。
**同一段冻结整段只算一次账,与冻结时长无关(不做任何时长累加)**。
...
另补一行**诊断日志**(`freeze episode: bufS=… held at …p — water-level downgrade suppressed`):
水位急救此前是唯一静默生效的路径,被闸住时日志里什么都没发生,无法区分「闸生效」与「ABR 没在评估」
(本次 144p 锁死复盘就吃过这个亏)。
本例预期:止步 480p 一档,缓冲 4s 回满 → episode 清位 → 恢复正常升降。
...
- 日志应见 `freeze episode: … suppressed` 一行(每 episode 一次),且**同段冻结只出现一次**
  `buffer-critical downgrade`。
- 级联场景下 `downgrade fail cooldown` 应**只记一笔**(源档),不再有 240p/360p/480p 沿途各一笔。
- ①的服务端改推 335 若复现,本修法**不能**阻止第一次降档(那是正确反应),但应止步一档。
